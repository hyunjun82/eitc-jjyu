import type { HubArticle, SpokeArticle } from "@/lib/types";

export const hub: HubArticle = {
  categorySlug: "payment",
  title: "2026 근로장려금 지급액 | 가구유형별 최대 금액·계산방법",
  h1: "2026 근로장려금 지급액 | 가구유형별 최대 금액·계산방법",
  metaDescription:
    "2026년 근로장려금 지급액을 가구유형별로 안내해요. 단독가구 최대 165만 원, 홑벌이 285만 원, 맞벌이 330만 원 및 계산방법을 확인하세요.",
  description:
    "근로장려금은 가구유형과 소득 수준에 따라 지급액이 달라져요. 각 가구별 최대 금액과 산정 방법을 알아보세요.",
  heroDescription:
    "내가 받을 수 있는 근로장려금은 얼마일까요? 가구유형별 최대 지급액과 정확한 계산방법을 확인해 보세요!",
  spokes: [
    {
      slug: "single",
      title: "단독가구 근로장려금 지급액",
      description:
        "단독가구의 근로장려금 최대 165만 원과 소득구간별 산정 방법을 안내해요.",
    },
    {
      slug: "single-income",
      title: "홑벌이가구 근로장려금 지급액",
      description:
        "홑벌이가구의 근로장려금 최대 285만 원과 소득구간별 산정 방법을 안내해요.",
    },
    {
      slug: "dual-income",
      title: "맞벌이가구 근로장려금 지급액",
      description:
        "맞벌이가구의 근로장려금 최대 330만 원과 소득구간별 산정 방법을 안내해요.",
    },
    {
      slug: "calculation",
      title: "근로장려금 계산방법 상세 가이드",
      description:
        "근로장려금 산정 공식과 계산 방법을 예시와 함께 자세히 설명해요.",
    },
    {
      slug: "simulator",
      title: "근로장려금 모의계산 가이드",
      description:
        "근로장려금 예상 수령액을 직접 계산해 보는 방법과 모의계산 도구 활용법을 안내해요.",
    },
  ],
  datePublished: "2026-02-24",
  dateModified: "2026-02-25",
};

export const spokes: Record<string, SpokeArticle> = {
  single: {
    slug: "single",
    categorySlug: "payment",
    title: "단독가구 근로장려금 지급액 | 최대 165만 원 산정 기준",
    h1: "단독가구 근로장려금 지급액 | 최대 165만 원 산정 기준",
    metaDescription:
      "2026년 단독가구 근로장려금 최대 165만 원의 산정 기준을 안내해요. 소득구간별 지급액과 계산 예시를 확인하세요.",
    description:
      "단독가구의 근로장려금은 총소득에 따라 최대 165만 원까지 받을 수 있어요. 소득구간별 산정 기준을 알아보세요.",
    heroDescription:
      "단독가구 근로장려금, 내 소득에서 얼마나 받을 수 있을까요? 최대 165만 원의 산정 기준과 계산 예시를 확인하세요!",
    faq: [
      {
        question: "단독가구 근로장려금 최대 금액은 얼마인가요?",
        answer:
          "2026년 기준 단독가구의 근로장려금 최대 지급액은 165만 원이에요. 총급여액 등이 400만 원~900만 원 구간에서 최대 금액을 받을 수 있어요.",
      },
      {
        question: "단독가구인데 소득이 낮으면 장려금도 적게 받나요?",
        answer:
          "네, 소득이 400만 원 미만이면 소득에 비례하여 장려금이 산정되므로 최대 금액보다 적게 받을 수 있어요. 이를 '점증구간'이라고 해요. 소득이 너무 낮으면 장려금도 적어지는 구조예요.",
      },
      {
        question: "단독가구 소득이 2,200만 원에 가까우면 얼마를 받나요?",
        answer:
          "소득이 900만 원을 초과하면 '점감구간'에 들어가 소득이 높아질수록 장려금이 줄어들어요. 2,200만 원에 가까운 소득이면 장려금이 매우 적거나 0원에 가까울 수 있어요.",
      },
    ],
    sections: [
      {
        title: "단독가구 소득구간별 지급액 기준",
        content:
          "단독가구의 근로장려금은 총급여액 등(총소득)에 따라 세 구간으로 나뉘어 산정돼요. 점증구간, 평탄구간, 점감구간으로 구성되며, 각 구간별로 지급액 계산 방식이 달라요.\n\n점증구간은 총급여액 등이 0원 초과~400만 원 미만인 구간이에요. 이 구간에서는 소득이 증가할수록 장려금도 늘어나요. 총급여액에 일정 비율(400분의 165)을 곱하여 산정해요. 예를 들어 총급여가 200만 원이면 약 82만 5천 원을 받을 수 있어요.\n\n평탄구간은 총급여액 등이 400만 원 이상~900만 원 미만인 구간으로, 최대 금액인 165만 원을 받을 수 있는 구간이에요. 이 소득 구간에 해당하면 소득에 관계없이 동일하게 165만 원이 지급돼요.",
        visual: {
          type: "incomeRange",
          items: [
            { label: "점증구간", value: 400, color: "blue", subLabel: "0~400만 원" },
            { label: "평탄구간", value: 500, color: "green", subLabel: "400~900만 원" },
            { label: "점감구간", value: 1300, color: "amber", subLabel: "900~2,200만 원" },
          ],
        },
        infoItems: [
          {
            type: "소득구간",
            name: "점증구간 (0~400만 원)",
            amount: "최대 165만 원까지 증가",
            role: "총급여액 × (165/400)",
          },
          {
            type: "소득구간",
            name: "평탄구간 (400~900만 원)",
            amount: "165만 원 (최대)",
            role: "일정하게 최대 금액 지급",
          },
          {
            type: "소득구간",
            name: "점감구간 (900~2,200만 원)",
            amount: "165만 원에서 0원으로 감소",
            role: "165만 원 - (총급여액-900만 원) × (165/1,300)",
          },
        ],
      },
      {
        title: "점증구간 상세 설명",
        content:
          "점증구간(0원~400만 원)은 소득이 적을수록 장려금도 적게, 소득이 늘수록 장려금도 많이 받는 구간이에요. 이 구간의 취지는 '일할수록 더 많은 혜택'을 제공하여 근로 의욕을 높이는 것이에요.\n\n계산 공식은 '총급여액 등 × (165만 원 ÷ 400만 원)'이에요. 쉽게 말해 총급여 100만 원당 약 41만 2,500원의 장려금이 산정돼요. 총급여가 100만 원이면 약 41만 원, 200만 원이면 약 82만 원, 300만 원이면 약 124만 원 정도예요.\n\n이 구간에서 주의할 점은 소득이 너무 낮으면 장려금도 그만큼 적다는 거예요. 예를 들어 연간 총급여가 50만 원이라면 장려금은 약 20만 6천 원 정도에 불과해요. 따라서 파트타임이나 단기 근무로 소득이 매우 적은 경우에는 기대보다 적은 금액을 받을 수 있어요.",
      },
      {
        title: "평탄구간과 점감구간",
        content:
          "평탄구간(400만 원~900만 원)은 단독가구에서 가장 유리한 소득 구간이에요. 이 구간에 해당하면 무조건 최대 금액인 165만 원을 받을 수 있어요. 연간 총급여가 500만 원이든 800만 원이든 동일하게 165만 원이 지급돼요.\n\n점감구간(900만 원~2,200만 원)은 소득이 높아질수록 장려금이 줄어드는 구간이에요. 계산 공식은 '165만 원 - (총급여액 등 - 900만 원) × (165만 원 ÷ 1,300만 원)'이에요. 소득 100만 원 증가할 때마다 약 12만 7천 원씩 장려금이 줄어들어요.\n\n점감구간의 끝인 총급여 2,200만 원에 도달하면 장려금은 0원이 돼요. 예를 들어 총급여가 1,500만 원이면 장려금은 약 88만 8천 원, 2,000만 원이면 약 25만 4천 원 정도예요. 소득이 기준금액에 가까울수록 장려금이 매우 적어지므로, 실제 수령액을 미리 계산해 보는 것이 좋아요.",
      },
      {
        title: "단독가구 지급액 계산 예시",
        content:
          "실제 사례를 통해 단독가구의 장려금을 계산해 볼게요. 먼저, 편의점 아르바이트로 연간 총급여 600만 원을 받는 A씨의 경우, 평탄구간(400만~900만 원)에 해당하므로 최대 금액인 165만 원을 받을 수 있어요.\n\n다음으로, 일용직으로 연간 총급여 1,200만 원을 받는 B씨의 경우, 점감구간에 해당해요. 계산하면 165만 원 - (1,200만 원 - 900만 원) × (165/1,300) = 165만 원 - 38만 1천 원 = 약 126만 9천 원이 산정돼요.\n\n마지막으로, 사업소득이 있는 C씨의 경우를 볼게요. C씨는 소매업을 운영하며 연간 수입금액이 2,000만 원이에요. 소매업 조정률 30%를 적용하면 총소득은 600만 원이 돼요. 이는 평탄구간에 해당하므로 C씨도 최대 165만 원을 받을 수 있어요. 사업소득 조정률 적용이 유리하게 작용한 사례예요.",
      },
      {
        title: "단독가구 지급액 관련 유의사항",
        content:
          "단독가구 장려금 수령 시 몇 가지 유의사항이 있어요. 먼저, 재산이 1.7억 원 이상 2.4억 원 미만인 경우 산정된 장려금의 50%만 지급돼요. 즉, 최대 165만 원이 산정되었더라도 재산이 1.7억 원 이상이면 실제 수령액은 82만 5천 원이 돼요.\n\n또한, 기한후 신청(6월~11월)을 하면 산정된 장려금에서 5~10% 감액돼요. 정기신청 기간(5월)에 반드시 신청하는 것이 유리해요. 체납 세금이 있는 경우에도 장려금에서 체납액을 차감한 후 지급되므로, 순수 수령액이 줄어들 수 있어요.\n\n단독가구는 주로 청년층, 중장년 1인 가구가 해당돼요. 특히 아르바이트, 프리랜서, 일용직 등 소득이 불안정한 분들에게 근로장려금은 중요한 소득 보전 수단이 될 수 있어요. 본인의 소득을 정확히 파악하고, 신청 기간을 놓치지 않는 것이 가장 중요해요.",
      },
    ],
    datePublished: "2026-02-24",
    dateModified: "2026-02-25",
  },
  "single-income": {
    slug: "single-income",
    categorySlug: "payment",
    title: "홑벌이가구 근로장려금 지급액 | 최대 285만 원 산정 기준",
    h1: "홑벌이가구 근로장려금 지급액 | 최대 285만 원 산정 기준",
    metaDescription:
      "2026년 홑벌이가구 근로장려금 최대 285만 원의 산정 기준을 안내해요. 소득구간별 지급액 계산 방법과 실제 예시를 확인하세요.",
    description:
      "홑벌이가구의 근로장려금은 총소득에 따라 최대 285만 원까지 받을 수 있어요. 소득구간별 산정 기준과 계산 예시를 안내해요.",
    heroDescription:
      "홑벌이가구라면 최대 285만 원까지 근로장려금을 받을 수 있어요! 내 소득에서 정확히 얼마를 받을 수 있는지 확인해 보세요.",
    faq: [
      {
        question: "홑벌이가구 근로장려금 최대 금액은 얼마인가요?",
        answer:
          "2026년 기준 홑벌이가구의 근로장려금 최대 지급액은 285만 원이에요. 총급여액 등이 700만 원~1,400만 원 구간에서 최대 금액을 받을 수 있어요.",
      },
      {
        question: "배우자 소득이 300만 원 미만이면 홑벌이가구인가요?",
        answer:
          "네, 배우자의 총급여액 등(근로소득 + 사업소득 + 종교인소득)이 300만 원 미만이면 홑벌이가구로 분류돼요. 300만 원 이상이면 맞벌이가구가 되어 기준과 지급액이 달라져요.",
      },
      {
        question: "한부모가구도 홑벌이가구에 해당하나요?",
        answer:
          "네, 배우자 없이 18세 미만 부양자녀가 있는 한부모가구는 홑벌이가구로 분류돼요. 단독가구보다 소득 기준과 최대 지급액이 높아서 더 많은 혜택을 받을 수 있어요.",
      },
    ],
    sections: [
      {
        title: "홑벌이가구 소득구간별 지급액 기준",
        content:
          "홑벌이가구의 근로장려금도 단독가구와 마찬가지로 점증구간, 평탄구간, 점감구간 세 단계로 나뉘어요. 다만, 각 구간의 소득 범위와 최대 금액이 더 크다는 차이가 있어요.\n\n점증구간은 총급여액 등이 0원 초과~700만 원 미만인 구간이에요. 소득이 증가할수록 장려금도 비례하여 늘어나요. 평탄구간은 700만 원~1,400만 원으로, 이 구간에서 최대 285만 원을 받을 수 있어요.\n\n점감구간은 1,400만 원~3,200만 원 구간으로, 소득이 높아질수록 장려금이 줄어들어요. 3,200만 원에 도달하면 장려금은 0원이 돼요. 홑벌이가구는 단독가구보다 소득 범위가 넓어서 더 많은 소득을 올리면서도 장려금을 받을 수 있어요.",
        visual: {
          type: "incomeRange",
          items: [
            { label: "점증구간", value: 700, color: "blue", subLabel: "0~700만 원" },
            { label: "평탄구간", value: 700, color: "green", subLabel: "700~1,400만 원" },
            { label: "점감구간", value: 1800, color: "amber", subLabel: "1,400~3,200만 원" },
          ],
        },
        infoItems: [
          {
            type: "소득구간",
            name: "점증구간 (0~700만 원)",
            amount: "최대 285만 원까지 증가",
            role: "총급여액 × (285/700)",
          },
          {
            type: "소득구간",
            name: "평탄구간 (700~1,400만 원)",
            amount: "285만 원 (최대)",
            role: "일정하게 최대 금액 지급",
          },
          {
            type: "소득구간",
            name: "점감구간 (1,400~3,200만 원)",
            amount: "285만 원에서 0원으로 감소",
            role: "285만 원 - (총급여액-1,400만 원) × (285/1,800)",
          },
        ],
      },
      {
        title: "점증구간과 평탄구간 상세",
        content:
          "점증구간(0~700만 원)에서는 총급여액에 비례하여 장려금이 산정돼요. 계산 공식은 '총급여액 등 × (285만 원 ÷ 700만 원)'이에요. 총급여 100만 원당 약 40만 7천 원의 장려금이 산정되는 셈이에요.\n\n예를 들어, 총급여가 350만 원이면 장려금은 약 142만 5천 원, 500만 원이면 약 203만 6천 원이 산정돼요. 소득이 적을수록 장려금도 적지만, 일하는 만큼 혜택이 늘어나는 구조여서 근로 동기부여 효과가 있어요.\n\n평탄구간(700만~1,400만 원)에서는 소득과 관계없이 최대 금액인 285만 원이 지급돼요. 이 구간은 근로장려금의 혜택이 가장 큰 '골든존'이에요. 연간 총급여가 800만 원이든 1,300만 원이든 동일하게 285만 원을 받을 수 있어요.",
      },
      {
        title: "점감구간 상세 설명",
        content:
          "점감구간(1,400만~3,200만 원)에서는 소득이 높아질수록 장려금이 줄어들어요. 계산 공식은 '285만 원 - (총급여액 등 - 1,400만 원) × (285만 원 ÷ 1,800만 원)'이에요.\n\n소득이 100만 원 증가할 때마다 약 15만 8천 원씩 장려금이 감소해요. 예를 들어, 총급여가 2,000만 원이면 장려금은 285만 원 - (600만 원 × 285/1,800) = 285만 원 - 95만 원 = 약 190만 원이 돼요.\n\n총급여가 2,800만 원이면 약 63만 3천 원, 3,100만 원이면 약 15만 8천 원으로 크게 줄어들어요. 점감구간의 끝인 3,200만 원에 도달하면 장려금은 0원이 돼요. 소득이 기준금액에 근접한 분들은 장려금이 매우 적어지므로, 실제 수령액을 미리 확인하는 것이 좋아요.",
      },
      {
        title: "홑벌이가구 계산 예시",
        content:
          "실제 사례를 통해 홑벌이가구의 장려금을 계산해 볼게요. D씨는 배우자와 자녀 1명이 있는 홑벌이가구로, 연간 총급여가 1,000만 원이에요. 평탄구간(700만~1,400만 원)에 해당하므로 최대 285만 원을 받을 수 있어요.\n\nE씨는 한부모가구로 18세 미만 자녀 2명을 양육하고 있어요. 연간 총급여가 1,800만 원인 E씨는 점감구간에 해당해요. 계산하면 285만 원 - (1,800만 원 - 1,400만 원) × (285/1,800) = 285만 원 - 63만 3천 원 = 약 221만 7천 원이 돼요.\n\nF씨는 배우자가 있고 70세 이상 어머니를 모시고 있어요. F씨의 연간 총급여는 400만 원이고 배우자 소득은 없어요. 점증구간에 해당하여 장려금은 400만 원 × (285/700) = 약 162만 9천 원이 산정돼요. 각 가구의 상황에 따라 지급액이 다르므로, 본인의 정확한 소득을 기준으로 계산해 보세요.",
      },
      {
        title: "홑벌이가구 지급액 극대화 팁",
        content:
          "홑벌이가구가 근로장려금을 최대한 많이 받으려면 몇 가지 포인트를 알아두면 좋아요. 가장 중요한 것은 총급여액이 평탄구간(700만~1,400만 원)에 속하도록 소득 수준을 파악하는 거예요. 이 구간이면 최대 285만 원을 온전히 받을 수 있어요.\n\n재산 관리도 중요해요. 재산이 1.7억 원 이상이면 장려금이 50% 감액되므로, 285만 원이 아닌 142만 5천 원만 받게 돼요. 재산 기준일(6월 1일) 전에 재산 현황을 점검해 보세요.\n\n홑벌이가구 중 한부모가구는 자녀장려금도 함께 신청할 수 있어요. 근로장려금과 자녀장려금은 별도로 산정되므로, 두 가지를 모두 신청하면 더 많은 지원을 받을 수 있어요. 자녀장려금은 부양자녀 1인당 최대 100만 원이 지급되니, 꼭 함께 신청하세요.",
      },
    ],
    datePublished: "2026-02-24",
    dateModified: "2026-02-25",
  },
  "dual-income": {
    slug: "dual-income",
    categorySlug: "payment",
    title: "맞벌이가구 근로장려금 지급액 | 최대 330만 원 산정 기준",
    h1: "맞벌이가구 근로장려금 지급액 | 최대 330만 원 산정 기준",
    metaDescription:
      "2026년 맞벌이가구 근로장려금 최대 330만 원의 산정 기준을 안내해요. 소득구간별 지급액 계산 방법과 실제 예시를 확인하세요.",
    description:
      "맞벌이가구의 근로장려금은 총소득에 따라 최대 330만 원까지 받을 수 있어요. 부부 합산 소득 기준 산정 방법을 안내해요.",
    heroDescription:
      "맞벌이가구라면 근로장려금 최대 330만 원! 부부 합산 소득에 따른 정확한 지급액을 확인해 보세요.",
    faq: [
      {
        question: "맞벌이가구 근로장려금 최대 금액은 얼마인가요?",
        answer:
          "2026년 기준 맞벌이가구의 근로장려금 최대 지급액은 330만 원이에요. 부부 합산 총급여액 등이 800만 원~1,700만 원 구간에서 최대 금액을 받을 수 있어요.",
      },
      {
        question: "맞벌이가구의 소득은 부부 합산인가요?",
        answer:
          "네, 맞벌이가구의 총소득은 신청인과 배우자의 소득을 합산하여 계산해요. 부부 합산 소득이 3,800만 원 미만이어야 신청 자격이 있고, 합산 소득을 기준으로 장려금을 산정해요.",
      },
      {
        question: "맞벌이가구가 홑벌이가구보다 항상 유리한가요?",
        answer:
          "반드시 그렇지는 않아요. 맞벌이가구의 최대 지급액(330만 원)이 홑벌이(285만 원)보다 높지만, 소득 기준도 다르기 때문에 개별 상황에 따라 다를 수 있어요. 배우자 소득이 300만 원 전후라면 어느 쪽이 유리한지 비교해 볼 필요가 있어요.",
      },
    ],
    sections: [
      {
        title: "맞벌이가구 소득구간별 지급액 기준",
        content:
          "맞벌이가구의 근로장려금은 부부 합산 총급여액 등을 기준으로 점증구간, 평탄구간, 점감구간에 따라 산정돼요. 세 가구유형 중 최대 지급액이 가장 높은 330만 원이에요.\n\n점증구간은 총급여액 등이 0원 초과~800만 원 미만, 평탄구간은 800만 원~1,700만 원, 점감구간은 1,700만 원~3,800만 원이에요. 부부 합산 소득이므로 각 구간의 범위가 다른 가구유형보다 넓어요.\n\n맞벌이가구는 부부 모두가 경제활동을 하므로 합산 소득이 높아지기 쉬워요. 그래서 소득 기준금액도 3,800만 원으로 가장 높게 설정되어 있어요. 하지만 그만큼 점감구간도 넓어서, 소득이 높아질수록 장려금이 점차 줄어드는 폭이 커요.",
        visual: {
          type: "incomeRange",
          items: [
            { label: "점증구간", value: 800, color: "blue", subLabel: "0~800만 원" },
            { label: "평탄구간", value: 900, color: "green", subLabel: "800~1,700만 원" },
            { label: "점감구간", value: 2100, color: "amber", subLabel: "1,700~3,800만 원" },
          ],
        },
        infoItems: [
          {
            type: "소득구간",
            name: "점증구간 (0~800만 원)",
            amount: "최대 330만 원까지 증가",
            role: "총급여액 × (330/800)",
          },
          {
            type: "소득구간",
            name: "평탄구간 (800~1,700만 원)",
            amount: "330만 원 (최대)",
            role: "일정하게 최대 금액 지급",
          },
          {
            type: "소득구간",
            name: "점감구간 (1,700~3,800만 원)",
            amount: "330만 원에서 0원으로 감소",
            role: "330만 원 - (총급여액-1,700만 원) × (330/2,100)",
          },
        ],
      },
      {
        title: "점증구간과 평탄구간 상세",
        content:
          "점증구간(0~800만 원)에서는 부부 합산 총급여액에 비례하여 장려금이 증가해요. 계산 공식은 '총급여액 등 × (330만 원 ÷ 800만 원)'이에요. 총급여 100만 원당 약 41만 3천 원의 장려금이 산정돼요.\n\n예를 들어, 부부 합산 총급여가 400만 원이면 장려금은 약 165만 원, 600만 원이면 약 247만 5천 원이 돼요. 점증구간에서는 부부가 함께 일할수록 더 많은 장려금을 받을 수 있으므로, 근로 의욕 향상에 도움이 돼요.\n\n평탄구간(800만~1,700만 원)에서는 부부 합산 소득과 관계없이 최대 330만 원이 지급돼요. 맞벌이가구에서 가장 혜택이 큰 구간이에요. 부부 각자 월급이 40만~70만 원 수준인 저소득 맞벌이가구가 이 구간에 많이 해당돼요.",
      },
      {
        title: "점감구간 상세 설명",
        content:
          "점감구간(1,700만~3,800만 원)은 맞벌이가구에서 가장 넓은 소득 구간이에요. 2,100만 원의 구간 폭에 걸쳐 장려금이 330만 원에서 0원까지 점차 줄어들어요.\n\n계산 공식은 '330만 원 - (총급여액 등 - 1,700만 원) × (330만 원 ÷ 2,100만 원)'이에요. 소득 100만 원 증가당 약 15만 7천 원씩 장려금이 줄어드는 셈이에요.\n\n구체적인 예시를 보면, 부부 합산 총급여가 2,500만 원이면 장려금은 330만 원 - (800만 원 × 330/2,100) = 330만 원 - 125만 7천 원 = 약 204만 3천 원이에요. 3,000만 원이면 약 125만 7천 원, 3,500만 원이면 약 47만 1천 원으로 크게 줄어들어요. 점감구간의 끝인 3,800만 원이 되면 장려금은 0원이에요.",
      },
      {
        title: "맞벌이가구 계산 예시",
        content:
          "실제 사례로 계산해 볼게요. G씨 부부는 남편 연간 총급여 1,200만 원, 아내 총급여 500만 원으로 합산 1,700만 원이에요. 평탄구간의 상한인 1,700만 원이므로 최대 금액인 330만 원을 받을 수 있어요.\n\nH씨 부부는 남편 총급여 1,500만 원, 아내 총급여 800만 원으로 합산 2,300만 원이에요. 점감구간에 해당하며, 330만 원 - (2,300만 원 - 1,700만 원) × (330/2,100) = 330만 원 - 94만 3천 원 = 약 235만 7천 원이 산정돼요.\n\nI씨 부부는 남편이 프리랜서(서비스업)로 연수입 2,000만 원, 아내가 직장인으로 연급여 800만 원이에요. 남편의 사업소득은 조정률 60%를 적용하여 1,200만 원으로, 합산 총소득은 2,000만 원이에요. 점감구간에 해당하여 약 282만 9천 원이 산정돼요. 사업소득 조정률 덕분에 실소득보다 낮게 산정되어 유리한 경우예요.",
      },
      {
        title: "맞벌이가구 특별 유의사항",
        content:
          "맞벌이가구에서 가장 중요한 것은 부부의 소득 분류예요. 배우자의 총급여액 등이 300만 원 미만이면 홑벌이가구, 300만 원 이상이면 맞벌이가구로 분류돼요. 경계에 있는 경우 어느 쪽이 더 유리한지 비교해 봐야 해요.\n\n예를 들어, 신청인 총급여 1,000만 원, 배우자 총급여 300만 원인 경우를 살펴볼게요. 맞벌이가구로 분류되면 합산 소득 1,300만 원으로 평탄구간에 해당하여 330만 원을 받을 수 있어요. 반면 배우자 소득이 299만 원이면 홑벌이가구가 되어, 신청인 소득 1,000만 원 기준으로 285만 원을 받게 돼요. 이 경우 맞벌이가 유리해요.\n\n또한, 맞벌이가구는 부부 중 한 명만 신청해요. 부부가 각각 따로 신청하는 것이 아니라, 한 가구로서 한 번만 신청하면 돼요. 누가 신청하든 결과는 동일하지만, 환급 계좌는 신청인 본인 명의여야 하므로 이 점만 주의하세요.",
      },
    ],
    datePublished: "2026-02-24",
    dateModified: "2026-02-25",
  },
  calculation: {
    slug: "calculation",
    categorySlug: "payment",
    title: "근로장려금 계산방법 상세 가이드 | 산정 공식과 예시",
    h1: "근로장려금 계산방법 상세 가이드 | 산정 공식과 예시",
    metaDescription:
      "근로장려금 계산방법을 상세히 안내해요. 가구유형별 산정 공식, 점증·평탄·점감구간 계산법, 감액 요인까지 한눈에 확인하세요.",
    description:
      "근로장려금이 정확히 얼마나 나오는지 궁금하시죠? 산정 공식과 감액 요인, 실제 계산 예시를 통해 알아보세요.",
    heroDescription:
      "내 근로장려금, 정확히 얼마일까요? 산정 공식부터 감액 요인까지, 계산방법을 완벽히 이해할 수 있도록 안내해요!",
    faq: [
      {
        question: "근로장려금 계산기를 이용할 수 있나요?",
        answer:
          "네, 국세청 홈택스에서 '근로장려금 미리보기' 서비스를 통해 예상 금액을 계산할 수 있어요. 매년 9월경 오픈되며, 소득과 재산 정보를 입력하면 자동으로 산정돼요.",
      },
      {
        question: "근로장려금에서 감액되는 경우는 어떤 건가요?",
        answer:
          "재산이 1.7억 원 이상이면 50% 감액, 기한후 신청 시 5~10% 감액, 체납 세금이 있으면 체납액 차감 후 지급돼요. 이런 감액 요인을 고려해야 실제 수령액을 정확히 알 수 있어요.",
      },
      {
        question: "사업소득과 근로소득이 함께 있으면 어떻게 계산하나요?",
        answer:
          "근로소득은 총급여액, 사업소득은 총수입금액에 업종별 조정률을 곱한 금액을 합산하여 총소득을 산출해요. 이 합산 총소득을 기준으로 가구유형에 맞는 산정표를 적용해요.",
      },
    ],
    sections: [
      {
        title: "근로장려금 산정 기본 구조",
        content:
          "근로장려금은 '총급여액 등'을 기준으로 가구유형별 산정표에 따라 계산돼요. 모든 가구유형에서 공통적으로 점증구간, 평탄구간, 점감구간의 세 단계 구조를 따라요.\n\n점증구간에서는 소득이 증가할수록 장려금도 늘어나요. 이는 근로 의욕을 높이기 위한 설계예요. 평탄구간에서는 최대 금액이 일정하게 유지돼요. 점감구간에서는 소득이 높아질수록 장려금이 줄어들어, 일정 소득 이상에서는 자력으로 생활이 가능하다고 보는 거예요.\n\n이 구조를 그래프로 그리면 사다리꼴 모양이 돼요. 왼쪽 경사(점증), 평평한 꼭대기(평탄), 오른쪽 경사(점감)의 형태예요. 가구유형별로 각 구간의 소득 범위와 최대 금액만 다르고, 기본 구조는 동일해요.",
        visual: {
          type: "barCompare",
          items: [
            { label: "단독가구", value: 165, color: "blue", subLabel: "최대 165만 원" },
            { label: "홑벌이가구", value: 285, color: "green", subLabel: "최대 285만 원" },
            { label: "맞벌이가구", value: 330, color: "violet", subLabel: "최대 330만 원" },
          ],
        },
        infoItems: [
          {
            type: "가구유형",
            name: "단독가구",
            amount: "최대 165만 원",
            role: "점증 0~400만 / 평탄 400~900만 / 점감 900~2,200만",
          },
          {
            type: "가구유형",
            name: "홑벌이가구",
            amount: "최대 285만 원",
            role: "점증 0~700만 / 평탄 700~1,400만 / 점감 1,400~3,200만",
          },
          {
            type: "가구유형",
            name: "맞벌이가구",
            amount: "최대 330만 원",
            role: "점증 0~800만 / 평탄 800~1,700만 / 점감 1,700~3,800만",
          },
        ],
      },
      {
        title: "총급여액 등 산출 방법",
        content:
          "장려금 계산의 첫 단계는 '총급여액 등'을 산출하는 거예요. 이는 근로소득, 사업소득, 종교인소득을 합산한 금액이에요. 각 소득 유형별로 산출 방식이 달라요.\n\n근로소득은 연간 총급여액(비과세소득 제외)을 그대로 사용해요. 여러 직장에서 근로소득이 있다면 모두 합산해요. 사업소득은 총수입금액에 업종별 조정률을 곱한 금액을 사용해요. 종교인소득은 종교관련종사자소득의 총수입금액을 사용해요.\n\n주의할 점은 '총급여액 등'과 '총소득'은 다른 개념이라는 거예요. 총급여액 등은 장려금 산정에 사용되고, 총소득은 소득요건 판단에 사용돼요. 총소득에는 이자·배당·연금·기타소득이 추가로 포함돼요. 즉, 이자·배당소득은 소득요건 판단에는 반영되지만 장려금 산정 금액에는 포함되지 않아요.",
      },
      {
        title: "감액 요인 총정리",
        content:
          "산정표에 따라 계산된 장려금에서 추가로 감액되는 요인이 있어요. 이를 반영해야 실제 수령액을 정확히 알 수 있어요.\n\n첫 번째 감액 요인은 재산이에요. 가구원 재산 합계가 1억 7천만 원 이상 2억 4천만 원 미만이면 산정된 장려금의 50%만 지급돼요. 예를 들어 단독가구 최대 165만 원이 산정되었는데 재산이 1.8억 원이면, 실제 수령액은 82만 5천 원이에요.\n\n두 번째는 기한후 신청 감액이에요. 정기신청 기간(5월)을 놓치고 6월~11월에 기한후 신청을 하면, 산정된 장려금에서 5~10%가 감액돼요. 세 번째는 체납 차감이에요. 국세 체납액이 있으면 장려금에서 체납액을 차감(최대 30%)한 후 나머지를 지급해요. 이 세 가지 감액 요인을 모두 고려해야 최종 수령액을 정확히 계산할 수 있어요.",
        visual: {
          type: "segment",
          items: [
            { label: "전액 지급", value: 170, color: "green", subLabel: "재산 1.7억 미만" },
            { label: "50% 감액", value: 70, color: "amber", subLabel: "1.7~2.4억" },
            { label: "대상 제외", value: 60, color: "red", subLabel: "2.4억 이상" },
          ],
        },
      },
      {
        title: "종합 계산 예시",
        content:
          "모든 요소를 종합하여 최종 수령액을 계산하는 예시를 보여드릴게요. J씨는 홑벌이가구(배우자 + 자녀 1명)로, 연간 총급여 1,000만 원(근로소득), 가구원 재산 합계 1.9억 원, 체납 세금 없음이에요.\n\n1단계: 총급여액 등 산출 → 1,000만 원(근로소득만 있음)\n2단계: 산정표 적용 → 평탄구간(700만~1,400만 원)에 해당하므로 285만 원\n3단계: 재산 감액 → 재산 1.9억 원(1.7억 원 이상)이므로 50% 감액 → 142만 5천 원\n4단계: 기타 감액 → 정기신청, 체납 없음 → 추가 감액 없음\n최종 수령액: 142만 5천 원\n\nK씨는 맞벌이가구로, 본인 총급여 1,200만 원, 배우자 총급여 500만 원(합산 1,700만 원), 재산 1.2억 원, 기한후 신청(10% 감액)이에요. 평탄구간에 해당하여 330만 원이 산정되고, 재산 감액은 없지만 기한후 신청으로 10% 감액되어 최종 297만 원을 수령해요.",
      },
      {
        title: "계산 시 자주 하는 실수",
        content:
          "근로장려금 계산에서 가장 흔한 실수는 총소득과 총급여액 등을 혼동하는 거예요. 소득요건 판단 시 사용하는 총소득에는 이자·배당·연금소득이 포함되지만, 장려금 산정 시 사용하는 총급여액 등에는 포함되지 않아요. 이 차이를 모르면 계산 결과가 달라질 수 있어요.\n\n두 번째 실수는 사업소득 조정률을 적용하지 않는 거예요. 사업소득자는 총수입금액이 아닌 조정된 금액을 사용해야 해요. 조정률을 적용하지 않으면 총소득이 과대 산출되어 자격이 없다고 오해하거나, 장려금이 적게 계산될 수 있어요.\n\n세 번째 실수는 재산 감액을 고려하지 않는 거예요. 재산이 1.7억 원 이상이면 장려금이 절반으로 줄어드는데, 이를 빠트리고 전액을 기대하면 실제 지급액에서 실망할 수 있어요. 홈택스의 '근로장려금 미리보기' 서비스를 이용하면 이런 실수 없이 정확한 예상 금액을 확인할 수 있으니, 적극 활용하시는 것을 추천해요.",
      },
    ],
    datePublished: "2026-02-24",
    dateModified: "2026-02-25",
  },
  simulator: {
    slug: "simulator",
    categorySlug: "payment",
    title: "근로장려금 모의계산 가이드 | 예상 수령액 계산기 활용법",
    h1: "근로장려금 모의계산 가이드 | 예상 수령액 계산기 활용법",
    metaDescription:
      "근로장려금 예상 수령액을 미리 계산해 보세요. 홈택스 미리보기 서비스, 직접 계산 방법, 감액 요인 반영까지 모의계산 완벽 가이드를 안내해요.",
    description:
      "근로장려금을 신청하기 전에 예상 수령액을 미리 계산해 볼 수 있어요. 모의계산 방법과 활용 팁을 안내해요.",
    heroDescription:
      "내가 받을 근로장려금이 정확히 얼마인지 궁금하시죠? 모의계산으로 미리 확인하고 신청하세요!",
    faq: [
      {
        question: "모의계산 결과와 실제 지급액이 다를 수 있나요?",
        answer:
          "네, 모의계산은 예상 금액이므로 실제 지급액과 차이가 있을 수 있어요. 국세청이 최종 결정 시 확인하는 소득·재산 자료가 본인이 입력한 것과 다를 수 있고, 기한후 신청 감액이나 체납 차감 등 추가 감액 요인이 반영되면 실제 수령액이 달라질 수 있어요.",
      },
      {
        question: "사업소득이 있으면 모의계산에서 조정률을 어떻게 반영하나요?",
        answer:
          "사업소득은 총수입금액에 업종별 조정률을 곱한 금액을 사용해요. 예를 들어 소매업 조정률 30%, 음식점업 45%, 서비스업 60% 등이에요. 모의계산 시 총수입금액이 아닌 조정률 적용 후 금액을 총급여액 등에 합산해야 정확한 결과를 얻을 수 있어요.",
      },
      {
        question: "모의계산 결과가 0원이면 신청할 수 없는 건가요?",
        answer:
          "모의계산 결과가 0원이면 소득이 기준금액을 초과했거나 다른 자격 요건을 충족하지 못한 것일 수 있어요. 다만, 소득·재산 정보를 잘못 입력한 경우일 수도 있으니 입력값을 다시 확인해 보세요. 정확한 판단은 홈택스 미리보기 서비스나 세무서 상담을 통해 확인하는 것이 좋아요.",
      },
    ],
    sections: [
      {
        title: "모의계산이 필요한 이유",
        content:
          "근로장려금을 신청하기 전에 예상 수령액을 미리 파악하는 것은 매우 중요해요. 소득 수준, 재산 규모, 가구유형에 따라 지급액이 천차만별이기 때문에 '내가 얼마를 받을 수 있는지' 사전에 확인해야 합리적인 재정 계획을 세울 수 있어요.\n\n가구유형별 최대 지급액은 단독가구 165만 원, 홑벌이가구 285만 원, 맞벌이가구 330만 원으로 큰 차이가 있어요. 같은 가구유형이라도 소득구간(점증·평탄·점감)에 따라 실제 지급액이 크게 달라지고, 재산 감액까지 적용되면 예상과 전혀 다른 금액을 받게 될 수 있어요.\n\n모의계산에는 크게 두 가지 방법이 있어요. 하나는 국세청 홈택스의 '근로장려금 미리보기' 서비스를 이용하는 것이고, 다른 하나는 산정 공식을 활용하여 직접 계산하는 방법이에요. 두 가지 방법을 모두 활용하면 더욱 정확한 예상 수령액을 파악할 수 있어요.",
        visual: {
          type: "barCompare",
          items: [
            { label: "단독가구", value: 165, color: "blue", subLabel: "최대 165만 원" },
            { label: "홑벌이가구", value: 285, color: "green", subLabel: "최대 285만 원" },
            { label: "맞벌이가구", value: 330, color: "violet", subLabel: "최대 330만 원" },
          ],
        },
      },
      {
        title: "홈택스 미리보기 서비스 활용",
        content:
          "국세청 홈택스에서는 매년 9월경 '근로장려금 미리보기' 서비스를 오픈해요. 홈택스에 접속한 뒤 '장려금' 메뉴에서 '미리보기'를 선택하면 이용할 수 있어요. 이 서비스는 국세청이 보유한 소득·재산 자료를 자동으로 반영하여 예상 장려금을 산출해 줘요.\n\n이용 방법은 간단해요. 홈택스에 공동인증서 또는 간편인증으로 로그인한 뒤, 미리보기 서비스에 접속하면 본인의 소득과 재산 정보가 자동으로 조회돼요. 별도의 정보를 입력할 필요 없이 로그인만 하면 예상 장려금 결과를 바로 확인할 수 있어서 매우 편리해요.\n\n다만, 미리보기 서비스의 결과는 어디까지나 '예상' 금액이에요. 실제 결정 시 반영되는 최종 소득·재산 자료와 차이가 있을 수 있고, 기한후 신청 감액이나 체납 차감 등은 미리보기에 반영되지 않을 수 있어요. 따라서 미리보기 결과를 참고하되, 실제 수령액과 다를 수 있다는 점을 감안하세요.",
        infoItems: [
          {
            type: "안내",
            name: "이용시기",
            amount: "매년 9월~",
            role: "국세청이 매년 9월경 서비스를 오픈해요",
          },
          {
            type: "안내",
            name: "경로",
            amount: "홈택스→장려금→미리보기",
            role: "홈택스 로그인 후 장려금 메뉴에서 접근해요",
          },
          {
            type: "안내",
            name: "필요정보",
            amount: "로그인만 하면 자동 조회",
            role: "소득·재산 정보가 자동으로 반영돼요",
          },
        ],
      },
      {
        title: "직접 계산하는 방법 (산정 공식)",
        content:
          "홈택스 미리보기 서비스를 이용할 수 없는 시기이거나 직접 계산해 보고 싶다면, 3단계 절차로 근로장려금을 산출할 수 있어요. 첫째, 본인의 가구유형(단독·홑벌이·맞벌이)을 확인하세요. 둘째, 총급여액 등(근로소득 + 사업소득 조정액 + 종교인소득)을 산출하세요. 셋째, 가구유형별 산정표를 적용하여 장려금을 계산하세요.\n\n산정 공식은 가구유형별로 다르게 적용돼요. 단독가구는 점증구간(0~400만 원)에서 총급여액 × (165/400), 평탄구간(400~900만 원)에서 165만 원, 점감구간(900~2,200만 원)에서 165만 원 - (총급여액-900만 원) × (165/1,300)이에요.\n\n홑벌이가구는 점증구간(0~700만 원)에서 총급여액 × (285/700), 평탄구간(700~1,400만 원)에서 285만 원, 점감구간(1,400~3,200만 원)에서 285만 원 - (총급여액-1,400만 원) × (285/1,800)이에요. 맞벌이가구는 점증구간(0~800만 원)에서 총급여액 × (330/800), 평탄구간(800~1,700만 원)에서 330만 원, 점감구간(1,700~3,800만 원)에서 330만 원 - (총급여액-1,700만 원) × (330/2,100)이에요.",
        infoItems: [
          {
            type: "산정공식",
            name: "단독가구",
            amount: "최대 165만 원",
            role: "점증: 총급여×(165/400) / 평탄: 165만 원 / 점감: 165-(총급여-900)×(165/1,300)",
          },
          {
            type: "산정공식",
            name: "홑벌이가구",
            amount: "최대 285만 원",
            role: "점증: 총급여×(285/700) / 평탄: 285만 원 / 점감: 285-(총급여-1,400)×(285/1,800)",
          },
          {
            type: "산정공식",
            name: "맞벌이가구",
            amount: "최대 330만 원",
            role: "점증: 총급여×(330/800) / 평탄: 330만 원 / 점감: 330-(총급여-1,700)×(330/2,100)",
          },
        ],
      },
      {
        title: "감액 요인 반영하기",
        content:
          "산정표에 따라 계산된 장려금이 곧 실수령액은 아니에요. 감액 요인을 반영해야 실제로 받게 될 금액을 정확히 파악할 수 있어요. 대표적인 감액 요인은 재산 감액, 기한후 신청 감액, 체납 차감 세 가지예요.\n\n재산 감액은 가구원 재산 합계가 1억 7천만 원 이상 2억 4천만 원 미만인 경우 산정된 장려금의 50%만 지급하는 제도예요. 재산이 2억 4천만 원 이상이면 아예 신청 자격이 없어요. 기한후 신청 감액은 정기신청 기간(5월)을 놓치고 6월~11월에 신청하면 5~10%가 감액돼요.\n\n체납 차감은 국세 체납액이 있을 경우 장려금에서 체납액을 차감하는 것으로, 최대 30%까지 차감될 수 있어요. 이 세 가지 감액 요인은 중복 적용될 수 있으므로, 모의계산 시 반드시 모두 확인하고 반영해야 실제 수령액에 가까운 결과를 얻을 수 있어요.",
        infoItems: [
          {
            type: "감액요인",
            name: "재산 감액",
            amount: "50% 감액",
            role: "가구원 재산 합계 1.7억 원 이상~2.4억 원 미만 시 적용",
          },
          {
            type: "감액요인",
            name: "기한후 신청 감액",
            amount: "5~10% 감액",
            role: "정기신청(5월) 기간 이후 6월~11월에 신청 시 적용",
          },
          {
            type: "감액요인",
            name: "체납 차감",
            amount: "최대 30% 차감",
            role: "국세 체납액이 있을 경우 장려금에서 차감 후 지급",
          },
        ],
      },
      {
        title: "모의계산 실전 예시 3가지",
        content:
          "예시 1: 단독가구 A씨, 총급여 600만 원, 재산 1억 원이에요. 총급여 600만 원은 평탄구간(400~900만 원)에 해당하므로 산정액은 최대 165만 원이에요. 재산이 1.7억 원 미만이므로 감액 없이 최종 수령액은 165만 원이에요.\n\n예시 2: 홑벌이가구 B씨, 총급여 1,800만 원, 재산 1.9억 원이에요. 총급여 1,800만 원은 점감구간(1,400~3,200만 원)에 해당해요. 산정액은 285만 원 - (1,800만 원 - 1,400만 원) × (285/1,800) = 285만 원 - 63만 3천 원 = 약 221만 7천 원이에요. 재산이 1.9억 원(1.7억 원 이상)이므로 50% 감액이 적용되어 최종 수령액은 약 110만 8천 원이에요.\n\n예시 3: 맞벌이가구 C씨 부부, 합산 총급여 2,500만 원, 재산 1.5억 원이에요. 총급여 2,500만 원은 점감구간(1,700~3,800만 원)에 해당해요. 산정액은 330만 원 - (2,500만 원 - 1,700만 원) × (330/2,100) = 330만 원 - 125만 7천 원 = 약 204만 3천 원이에요. 재산이 1.7억 원 미만이므로 감액 없이 최종 수령액은 약 204만 3천 원이에요.",
      },
    ],
    datePublished: "2026-02-25",
    dateModified: "2026-02-25",
  },
};
