import type { HubArticle, SpokeArticle } from "@/lib/types";

export const hub: HubArticle = {
  categorySlug: "comparisons",
  title: "근로장려금 비교분석 | 다른 복지제도·신청방식과의 차이점",
  h1: "근로장려금 비교분석 | 다른 복지제도·신청방식과의 차이점",
  metaDescription:
    "근로장려금(EITC)과 기초생활보장, 기초연금, 실업급여, 아동수당 등 다른 복지제도와의 차이점을 비교 분석해요. 중복 수급 가능 여부와 정기·반기 신청 방식 차이도 안내해요.",
  description:
    "근로장려금은 다른 복지제도와 어떻게 다를까요? 기초생활보장·기초연금·실업급여·아동수당과의 차이점, 중복 수급 가능 여부, 정기·반기 신청 방식 비교를 안내해요.",
  heroDescription:
    "근로장려금과 다른 복지제도의 차이가 헷갈리셨나요? 기초생활보장·기초연금·실업급여·아동수당과의 비교, 정기·반기 신청 방식까지 한눈에 정리했어요!",
  spokes: [
    {
      slug: "vs-basic-livelihood",
      title: "근로장려금 vs 기초생활보장 비교",
      description:
        "소득인정액 기준의 기초생활보장과 총소득 기준의 근로장려금 차이, 중복 수급 가능 여부를 비교해요.",
    },
    {
      slug: "vs-earned-income-credit",
      title: "근로장려금 vs 근로소득세액공제 차이",
      description:
        "환급형 세제 혜택인 근로장려금과 일반 세액공제인 근로소득세액공제의 핵심 차이를 안내해요.",
    },
    {
      slug: "vs-basic-pension",
      title: "근로장려금 vs 기초연금 비교",
      description:
        "65세 이상 기초연금과 근로장려금의 수급 요건·금액·중복 수급 여부를 비교해요.",
    },
    {
      slug: "vs-unemployment",
      title: "근로장려금 vs 실업급여 중복 수령",
      description:
        "실업급여 수급 중 근로장려금 신청 가능 여부와 소득 산정 방식의 차이를 안내해요.",
    },
    {
      slug: "vs-child-allowance",
      title: "근로장려금 vs 아동수당 비교",
      description:
        "아동수당·자녀장려금·근로장려금 세 제도의 지급 대상·금액·중복 수급 여부를 비교해요.",
    },
    {
      slug: "regular-vs-semi-detail",
      title: "정기 vs 반기 어떤 게 유리?",
      description:
        "근로장려금 정기신청과 반기신청의 지급 시기·금액·적합 대상을 비교해 유리한 방법을 안내해요.",
    },
  ],
  datePublished: "2026-02-26",
  dateModified: "2026-02-26",
};

export const spokes: Record<string, SpokeArticle> = {
  "vs-basic-livelihood": {
    slug: "vs-basic-livelihood",
    categorySlug: "comparisons",
    title: "근로장려금 vs 기초생활보장 비교 | 중복 수급 가능 여부와 소득 산정 차이",
    h1: "근로장려금 vs 기초생활보장 비교 | 중복 수급 가능 여부와 소득 산정 차이",
    metaDescription:
      "근로장려금과 기초생활보장(생계·의료·주거·교육급여)의 차이를 비교해요. 소득 기준, 중복 수급 가능 여부, 근로장려금이 소득인정액에 반영되는 구조를 확인하세요.",
    description:
      "근로장려금과 기초생활보장은 소득 기준 산정 방식이 달라요. 두 제도의 차이와 중복 수급 시 주의사항을 안내해요.",
    heroDescription:
      "기초생활수급자도 근로장려금을 받을 수 있을까요? 소득 기준 차이와 중복 수급 구조를 한눈에 비교해 보세요!",
    faq: [
      {
        question: "기초생활수급자도 근로장려금을 신청할 수 있나요?",
        answer:
          "네, 기초생활수급자도 근로장려금 소득·재산 요건을 충족하면 신청할 수 있어요. 다만, 수령한 근로장려금이 다음 연도 소득인정액에 반영되어 기초생활보장 급여가 줄어들 수 있어요. 사전에 복지로(www.bokjiro.go.kr)나 읍·면·동 주민센터에서 영향을 확인하는 것이 좋아요.",
      },
      {
        question: "기초생활보장과 근로장려금의 소득 기준은 어떻게 다른가요?",
        answer:
          "기초생활보장은 가구의 '소득인정액(소득평가액 + 재산의 소득환산액)'을 기준으로 수급 여부를 판단해요. 반면 근로장려금은 가구원의 '총소득(근로·사업·종교인·이자·배당·연금소득 합계)'을 기준으로 해요. 두 기준의 산정 방식이 달라, 기초생활보장 대상자이면서 동시에 근로장려금 대상자가 될 수 있어요.",
      },
      {
        question: "근로장려금을 받으면 기초생활보장 수급이 중단되나요?",
        answer:
          "근로장려금을 받는다고 해서 기초생활보장 수급이 자동으로 중단되지는 않아요. 다만, 수령한 근로장려금은 금융재산으로 인정되어 이듬해 소득인정액 계산에 반영될 수 있어요. 이로 인해 급여 금액이 소폭 줄어들 수 있지만, 수급 자격 자체가 박탈되는 경우는 드물어요.",
      },
    ],
    sections: [
      {
        title: "기초생활보장과 근로장려금은 어떤 제도인가요?",
        content:
          "기초생활보장은 소득인정액이 기준 중위소득 이하인 저소득 가구에 생계·의료·주거·교육급여를 지급하는 공공부조 제도로, 보건복지부가 운영해요. 2026년 기준 생계급여는 기준 중위소득 32% 이하, 의료급여는 40% 이하, 주거급여는 48% 이하, 교육급여는 50% 이하인 가구가 대상이에요. 가구의 소득뿐 아니라 재산(주거·금융·자동차 등)을 소득으로 환산한 '소득인정액'이 핵심 기준이에요.\n\n근로장려금(EITC)은 일하는 저소득 근로자·사업자를 대상으로 국세청이 세금 환급 방식으로 지급하는 제도예요. 2026년 기준 단독가구는 연 총소득 2,200만원 미만, 홑벌이가구는 3,200만원 미만, 맞벌이가구는 4,400만원 미만이면 신청 대상이에요. 재산 요건은 가구원 전체 재산 합계 2억4,000만원 미만이며, 1억7,000만원 이상이면 장려금이 50% 감액돼요.\n\n두 제도는 근거 법률, 운영 기관, 소득 산정 방식, 지급 방식이 모두 달라요. 기초생활보장은 '국민기초생활보장법'에 따라 보건복지부·읍면동 주민센터가 담당하고, 근로장려금은 '조세특례제한법'에 따라 국세청이 담당해요.",
        infoItems: [
          {
            type: "제도 비교",
            name: "기초생활보장",
            role: "보건복지부 운영, 소득인정액 기준, 생계·의료·주거·교육급여",
          },
          {
            type: "제도 비교",
            name: "근로장려금",
            role: "국세청 운영, 총소득 기준, 세금 환급형 현금 지급",
          },
          {
            type: "소득 기준",
            name: "기초생활보장",
            amount: "기준 중위소득 32~50%",
            role: "소득인정액(소득평가액 + 재산 소득환산액) 기준",
          },
          {
            type: "소득 기준",
            name: "근로장려금",
            amount: "단독 2,200만원 미만",
            role: "가구원 총소득(근로·사업·이자·배당·연금 합계) 기준",
          },
        ],
      },
      {
        title: "두 제도의 소득 기준은 어떻게 다른가요?",
        content:
          "기초생활보장의 소득인정액은 '소득평가액'과 '재산의 소득환산액'을 합산하여 산출해요. 소득평가액은 실제 소득에서 가구 특성별 지출 비용과 근로소득 공제를 뺀 금액이에요. 재산의 소득환산액은 부동산·금융재산·자동차 등의 재산에서 기본재산액과 부채를 뺀 후 소득환산율(주거용 재산 1.04%, 일반 재산 4.17%, 금융재산 6.26%, 자동차 100%)을 곱한 금액이에요.\n\n근로장려금의 총소득은 가구원 전체의 근로소득, 사업소득(농업·임업·어업 포함), 종교인소득, 이자소득, 배당소득, 연금소득을 모두 합산한 금액이에요. 비과세소득(실업급여, 산재급여 등)은 포함되지 않아요. 재산은 소득인정액처럼 소득으로 환산하지 않고, 재산 합계액이 2억4,000만원 미만인지를 별도로 판단해요.\n\n두 기준의 가장 큰 차이는 재산을 소득으로 환산하는지 여부예요. 기초생활보장은 재산도 소득처럼 환산하여 수급 자격에 영향을 주지만, 근로장려금은 재산이 2억4,000만원 미만이면 금액 산정에 영향을 주지 않아요. 이 차이로 인해 재산이 비교적 많아도 근로장려금은 받을 수 있는 경우가 생겨요.",
        infoItems: [
          {
            type: "소득 산정",
            name: "기초생활보장",
            role: "소득평가액 + 재산 소득환산액 = 소득인정액",
          },
          {
            type: "소득 산정",
            name: "근로장려금",
            role: "근로·사업·이자·배당·연금소득 합산 = 총소득 (재산 별도 판단)",
          },
          {
            type: "재산 처리",
            name: "기초생활보장",
            role: "재산 → 소득환산율 적용 → 소득인정액에 반영",
          },
          {
            type: "재산 처리",
            name: "근로장려금",
            amount: "2.4억원 미만",
            role: "재산 합계가 기준 이하이면 자격 유지 (1.7억원 이상 50% 감액)",
          },
        ],
      },
      {
        title: "두 제도를 동시에 받을 수 있나요?",
        content:
          "기초생활보장과 근로장려금은 법적으로 중복 수급이 가능해요. 기초생활보장 수급자라고 해서 근로장려금 신청이 금지되거나 자동으로 배제되지는 않아요. 소득·재산 요건만 충족하면 두 제도를 동시에 받을 수 있어요.\n\n다만, 중복 수급 시 주의할 점이 있어요. 수령한 근로장려금은 금융재산으로 인정되어, 다음 연도 기초생활보장 소득인정액 산정 시 금융재산으로 반영돼요. 금융재산 소득환산율이 월 6.26%로 높아서, 근로장려금 수령액이 클수록 다음 해 소득인정액이 높아져 급여가 줄어드는 효과가 생길 수 있어요. 예를 들어 165만원의 근로장려금을 받으면 다음 해 소득인정액에 월 약 8만6,000원이 추가로 반영될 수 있어요.\n\n의료급여 수급자의 경우 근로장려금 수령으로 의료급여 자격이 변동될 가능성이 있으므로, 수령 전에 읍·면·동 주민센터나 복지로 전화(129)를 통해 개인 상황에 맞는 영향을 사전에 확인하는 것을 권장해요. 소득이 매우 낮은 생계급여 1~2인 가구에서는 근로장려금 수령보다 급여 감액이 더 크게 발생할 수도 있으므로 반드시 사전 계산이 필요해요.",
      },
      {
        title: "어떤 경우에 두 제도를 동시에 받는 게 유리한가요?",
        content:
          "기초생활보장과 근로장려금을 동시에 받는 게 유리한 경우는 근로소득이 안정적으로 발생하면서도 기초생활보장 수급 자격을 유지하는 경우예요. 특히 주거급여와 교육급여 수급자는 기준이 상대적으로 완화되어 있고, 급여 감액 영향이 생계급여보다 작아 중복 수급이 유리한 경우가 많아요.\n\n근로장려금은 일하는 유인을 높이기 위해 설계된 제도예요. 기초생활보장 수급자가 근로소득을 얻게 되면 근로장려금을 추가로 받아 실질 소득을 높일 수 있어요. 기초생활보장의 근로소득 공제와 근로장려금을 함께 활용하면 빈곤 탈출에 도움이 될 수 있어요.\n\n반면, 의료급여 수급자는 급여 자격 변동이 의료비 부담을 크게 증가시킬 수 있어 신중하게 판단해야 해요. 생계급여만 받는 극빈층의 경우 근로장려금 수령 후 생계급여가 줄어드는 영향이 더 클 수 있으므로, 읍·면·동 주민센터의 사례관리사나 복지로 상담을 통해 개별 상황을 먼저 확인하는 게 좋아요.",
      },
    ],
    datePublished: "2026-02-26",
    dateModified: "2026-02-26",
  },

  "vs-earned-income-credit": {
    slug: "vs-earned-income-credit",
    categorySlug: "comparisons",
    title: "근로장려금 vs 근로소득세액공제 차이 | 환급형 세제와 일반 세액공제 비교",
    h1: "근로장려금 vs 근로소득세액공제 차이 | 환급형 세제와 일반 세액공제 비교",
    metaDescription:
      "근로장려금(EITC)과 근로소득세액공제의 핵심 차이를 비교해요. 환급형 vs 비환급형, 최대 금액, 동시 적용 가능 여부를 확인하세요.",
    description:
      "근로장려금은 세금보다 많이 돌려받을 수 있는 환급형 세제예요. 근로소득세액공제와 어떻게 다른지, 두 제도를 동시에 받을 수 있는지 안내해요.",
    heroDescription:
      "근로장려금과 근로소득세액공제, 둘 다 세금 관련 혜택인데 어떻게 다를까요? 핵심 차이와 동시 적용 여부를 쉽게 비교해 드려요!",
    faq: [
      {
        question: "근로장려금과 근로소득세액공제를 동시에 받을 수 있나요?",
        answer:
          "네, 두 제도는 별도의 법률에 따라 운영되므로 동시에 받을 수 있어요. 연말정산에서 근로소득세액공제를 적용받고, 5월에 근로장려금도 신청하면 두 혜택을 모두 누릴 수 있어요. 단, 근로장려금 신청 시 연말정산에서 환급받은 세액과 관계없이 독립적으로 산정돼요.",
      },
      {
        question: "근로소득세액공제는 누가 적용받나요?",
        answer:
          "근로소득세액공제는 근로소득이 있는 모든 근로자가 연말정산에서 자동으로 적용받아요. 산출세액의 55%(산출세액 130만원 이하) 또는 71만5,000원 + 130만원 초과분의 30%를 공제하며, 총급여 3,300만원 이하는 최대 74만원, 초과 시 공제한도가 단계적으로 축소돼요.",
      },
      {
        question: "세금이 0원이면 근로소득세액공제 혜택을 못 받나요?",
        answer:
          "근로소득세액공제는 산출세액(납부할 세금)에서 차감하는 방식이에요. 근로소득이 낮아 산출세액이 0원이면 세액공제 자체가 의미가 없어요. 반면 근로장려금은 산출세액이 0원이어도 소득·재산 요건만 충족하면 현금으로 지급돼요. 이것이 두 제도의 가장 큰 차이예요.",
      },
    ],
    sections: [
      {
        title: "근로장려금과 근로소득세액공제는 어떤 제도인가요?",
        content:
          "근로장려금(EITC, Earned Income Tax Credit)은 '조세특례제한법' 제100조의2에 따라 저소득 근로자·사업자에게 세금 환급 방식으로 지급하는 제도예요. 납부해야 할 세금이 없거나 세금보다 장려금이 더 많은 경우에도 그 차액을 현금으로 지급하는 '환급형(refundable)' 세액공제예요. 2026년 기준 최대 지급액은 단독가구 165만원, 홑벌이가구 285만원, 맞벌이가구 330만원이에요.\n\n근로소득세액공제는 '소득세법' 제59조에 따라 근로소득이 있는 모든 근로자가 산출세액에서 일정액을 공제받는 제도예요. 납부할 세금(산출세액)이 있을 때만 효과가 있는 '비환급형(non-refundable)' 세액공제예요. 산출세액이 130만원 이하이면 55%를 공제하고, 130만원을 초과하면 71만5,000원 + 초과분의 30%를 공제해요. 총급여 3,300만원 이하 근로자의 공제 한도는 연 최대 74만원이에요.\n\n두 제도의 핵심 차이는 '환급 여부'예요. 근로소득세액공제는 납부할 세금의 범위 내에서만 공제되지만, 근로장려금은 납부할 세금이 없어도 현금으로 받을 수 있어요. 따라서 소득이 낮아 세금이 거의 없는 근로자에게는 근로장려금의 실제 혜택이 훨씬 크게 느껴질 수 있어요.",
        infoItems: [
          {
            type: "제도 비교",
            name: "근로장려금",
            amount: "최대 330만원",
            role: "환급형, 세금이 없어도 현금 지급, 국세청 담당",
          },
          {
            type: "제도 비교",
            name: "근로소득세액공제",
            amount: "최대 74만원",
            role: "비환급형, 산출세액 범위 내 공제, 연말정산 자동 적용",
          },
          {
            type: "법적 근거",
            name: "근로장려금",
            role: "조세특례제한법 제100조의2",
          },
          {
            type: "법적 근거",
            name: "근로소득세액공제",
            role: "소득세법 제59조",
          },
        ],
      },
      {
        title: "두 제도의 공제 방식과 금액은 어떻게 다른가요?",
        content:
          "근로소득세액공제는 산출세액을 기준으로 공제율을 적용해요. 산출세액이 130만원 이하이면 55%를 공제하고, 130만원을 초과하면 71만5,000원에 초과분의 30%를 더해 공제해요. 공제 한도는 총급여 수준에 따라 달라져요. 총급여 3,300만원 이하이면 74만원, 3,300만원~7,000만원이면 74만원에서 총급여 3,300만원 초과분의 0.8%를 뺀 금액(최소 66만원), 7,000만원 초과이면 66만원에서 7,000만원 초과분의 50%를 뺀 금액(최소 50만원)이에요.\n\n근로장려금은 가구 유형과 총소득 구간에 따라 점증·유지·점감 구간으로 나뉘어 산정돼요. 단독가구를 예로 들면, 총소득 400만원 미만(점증 구간)에서는 총소득 × 165/400, 400만원 이상~900만원 미만(유지 구간)에서는 165만원, 900만원 이상~2,200만원 미만(점감 구간)에서는 165만원 × (2,200만원 - 총소득) / 1,300만원으로 계산돼요.\n\n실질적인 차이를 비교하면, 연간 총소득이 1,500만원인 단독가구 근로자의 경우 근로소득세액공제로 최대 74만원을 공제받을 수 있지만, 이 소득대에서 산출세액 자체가 매우 낮거나 0에 가까워 실제 혜택은 미미할 수 있어요. 반면 근로장려금은 이 소득대에서 수십만 원에서 최대 165만원까지 현금으로 받을 수 있어요.",
        infoItems: [
          {
            type: "공제 방식",
            name: "근로소득세액공제",
            role: "산출세액 130만원 이하: 55% 공제 / 130만원 초과: 71.5만원 + 초과분 30%",
          },
          {
            type: "공제 방식",
            name: "근로장려금 (단독가구)",
            role: "점증(총소득 × 165/400) → 유지(165만원) → 점감 구간 산정",
          },
          {
            type: "최대 한도",
            name: "근로소득세액공제",
            amount: "74만원",
            role: "총급여 3,300만원 이하 기준",
          },
          {
            type: "최대 한도",
            name: "근로장려금",
            amount: "330만원",
            role: "맞벌이가구 기준",
          },
        ],
      },
      {
        title: "두 제도를 동시에 적용받을 수 있나요?",
        content:
          "근로장려금과 근로소득세액공제는 동시에 적용받을 수 있어요. 두 제도는 법적 근거와 운영 기관이 다르고, 지급 요건과 산정 방식도 완전히 별개로 작동해요. 연말정산에서 근로소득세액공제를 적용받아 세금을 줄이거나 환급받은 후, 5월 정기신청(또는 9월·3월 반기신청)을 통해 근로장려금도 별도로 신청할 수 있어요.\n\n예를 들어, 총급여 1,200만원인 단독 근로자가 산출세액이 5만원이라고 가정하면, 근로소득세액공제로 5만원을 전액 공제(세금 0원)받고, 근로장려금으로 별도로 약 100만원 이상을 추가로 수령할 수 있어요. 두 혜택을 합산하면 실질적인 세제 지원 효과가 크게 높아져요.\n\n연말정산 시 근로소득세액공제가 먼저 적용되고, 이후 5월에 근로장려금을 별도로 신청하는 방식이에요. 단, 근로장려금은 전년도 소득을 기준으로 신청하는 제도이므로, 당해 연도 연말정산과 다음 해 5월 근로장려금 신청이 연계되어 있음을 이해해야 해요.",
      },
      {
        title: "어떤 제도가 나에게 더 유리한가요?",
        content:
          "두 제도는 비교 대상이 아니라 병행 활용할 수 있는 제도예요. 하지만 어느 제도에서 더 큰 혜택을 받는지는 소득 수준과 가구 유형에 따라 달라요. 소득이 낮아 세금을 거의 내지 않는 저소득 근로자라면, 근로소득세액공제의 실질 혜택은 작고 근로장려금에서 훨씬 큰 혜택을 받을 수 있어요.\n\n소득이 높아 산출세액이 많이 발생하는 경우, 근로소득세액공제의 효과가 커지는 반면 근로장려금은 점감 구간에 들어가거나 소득 요건을 초과해 받지 못하게 돼요. 2026년 기준 단독가구는 총소득 2,200만원, 홑벌이가구는 3,200만원, 맞벌이가구는 4,400만원을 초과하면 근로장려금 신청 자격이 없어요.\n\n결론적으로, 총소득이 2,000만원 이하인 저소득 근로자라면 근로장려금이 핵심 혜택이 되고, 근로소득세액공제는 추가적인 절세 수단이 돼요. 총소득이 근로장려금 한도 이상인 중간 소득 근로자라면 근로소득세액공제만 적용받게 돼요. 어느 경우든 두 제도를 동시에 챙기는 것이 최대의 세제 혜택을 받는 방법이에요.",
        infoItems: [
          {
            type: "유리한 상황",
            name: "저소득 근로자",
            role: "근로장려금이 핵심 + 근로소득세액공제 보조 활용",
          },
          {
            type: "유리한 상황",
            name: "중간 소득 근로자",
            role: "근로소득세액공제만 적용 (근로장려금 소득 초과 시)",
          },
          {
            type: "최적 전략",
            name: "동시 활용",
            role: "연말정산 세액공제 + 5월 근로장려금 별도 신청",
          },
        ],
      },
    ],
    datePublished: "2026-02-26",
    dateModified: "2026-02-26",
  },

  "vs-basic-pension": {
    slug: "vs-basic-pension",
    categorySlug: "comparisons",
    title: "근로장려금 vs 기초연금 비교 | 수급 요건·금액·중복 수급 여부 총정리",
    h1: "근로장려금 vs 기초연금 비교 | 수급 요건·금액·중복 수급 여부 총정리",
    metaDescription:
      "근로장려금과 기초연금의 수급 요건, 나이 기준, 지급 금액, 중복 수급 가능 여부를 비교해요. 65세 이상 노인의 두 제도 동시 수급 방법을 확인하세요.",
    description:
      "65세 이상 어르신이 기초연금과 근로장려금을 함께 받을 수 있는지, 두 제도의 요건과 금액이 어떻게 다른지 안내해요.",
    heroDescription:
      "기초연금 받는 어르신도 근로장려금을 신청할 수 있을까요? 두 제도의 수급 요건과 중복 수급 구조를 비교해 드려요!",
    faq: [
      {
        question: "기초연금을 받으면서 근로장려금도 신청할 수 있나요?",
        answer:
          "네, 기초연금과 근로장려금은 별개의 제도이므로 중복 수급이 가능해요. 65세 이상으로 기초연금을 받고 있어도, 근로·사업소득이 있고 총소득·재산 요건을 충족하면 근로장려금을 신청할 수 있어요. 기초연금 수령이 근로장려금 신청을 제한하지 않아요.",
      },
      {
        question: "기초연금은 근로장려금 소득 계산에 포함되나요?",
        answer:
          "아니요, 기초연금은 근로장려금의 총소득 산정에 포함되지 않아요. 근로장려금의 총소득에는 근로소득·사업소득·종교인소득·이자소득·배당소득·연금소득이 포함되는데, 여기서 '연금소득'은 국민연금 등 공적연금을 말하며 기초연금은 해당되지 않아요. 따라서 기초연금을 많이 받더라도 근로장려금 소득 요건에는 영향을 주지 않아요.",
      },
      {
        question: "근로장려금은 나이 제한이 있나요?",
        answer:
          "근로장려금에는 상한 나이 제한이 없어요. 다만 단독가구의 경우 신청 연도 12월 31일 기준 만 19세 이상이어야 해요. 65세 이상 어르신도 근로·사업소득이 있으면 신청할 수 있어요. 배우자가 있는 홑벌이·맞벌이 가구는 나이 제한이 따로 없어요.",
      },
    ],
    sections: [
      {
        title: "기초연금과 근로장려금은 어떤 제도인가요?",
        content:
          "기초연금은 '기초연금법'에 따라 만 65세 이상 국내 거주 노인 중 소득 하위 70%에 해당하는 분에게 보건복지부가 매월 지급하는 노후 소득 보장 제도예요. 2026년 기준 선정 기준액은 단독가구 월 228만원, 부부가구 월 364만8,000원이며(소득인정액 기준), 최대 지급액은 단독가구 기준 월 약 34만4,000원 수준이에요. 국민연금 수령액이 많으면 기초연금이 감액될 수 있어요.\n\n근로장려금은 만 19세 이상(단독가구 기준)으로 근로·사업소득이 있는 저소득층이 대상이에요. 상한 나이 제한이 없어 65세 이상 어르신도 소득·재산 요건을 충족하면 신청할 수 있어요. 2026년 기준 최대 지급액은 단독가구 165만원, 홑벌이가구 285만원, 맞벌이가구 330만원이며, 연 1회(정기) 또는 반기(상·하반기) 방식으로 수령할 수 있어요.\n\n두 제도의 가장 큰 차이는 나이 요건과 소득 발생 여부예요. 기초연금은 65세 이상이면 소득이 없어도 받을 수 있지만, 근로장려금은 나이와 무관하게 현재 근로·사업소득이 있어야 해요. 기초연금은 매월 지급되는 반면, 근로장려금은 연간 기준으로 지급돼요.",
        infoItems: [
          {
            type: "제도 비교",
            name: "기초연금",
            amount: "월 최대 약 34만4,000원",
            role: "만 65세 이상, 소득 하위 70%, 매월 지급",
          },
          {
            type: "제도 비교",
            name: "근로장려금",
            amount: "연 최대 330만원",
            role: "만 19세 이상(단독), 나이 상한 없음, 근로·사업소득 필수",
          },
          {
            type: "소득 기준",
            name: "기초연금",
            role: "소득인정액(단독 228만원 / 부부 364.8만원) 이하",
          },
          {
            type: "소득 기준",
            name: "근로장려금",
            role: "총소득(단독 2,200만원 / 홑벌이 3,200만원 / 맞벌이 4,400만원) 미만",
          },
        ],
      },
      {
        title: "기초연금이 근로장려금 수급에 영향을 주나요?",
        content:
          "기초연금 수령액은 근로장려금의 총소득 산정에 포함되지 않아요. 근로장려금의 총소득 계산 항목은 근로소득, 사업소득(농업·임업·어업 소득 포함), 종교인소득, 이자소득, 배당소득, 연금소득(공적연금)인데, 여기서 기초연금은 '연금소득'에 해당하지 않아요. 기초연금은 노인복지 관점에서 지급되는 급여로, 소득세법상 비과세소득이에요.\n\n따라서 매월 기초연금 34만4,000원을 받는 어르신도 근로장려금 총소득 산정 시 이 금액이 제외돼요. 예를 들어 70세 단독 어르신이 아르바이트로 연 600만원의 근로소득을 얻고 기초연금 연 413만원을 받는 경우, 근로장려금 산정용 총소득은 600만원만 인정돼요. 이 경우 유지 구간(400만원~900만원)에 해당하여 최대 165만원을 수령할 수 있어요.\n\n단, 기초연금이 근로장려금 재산 판정에 미치는 영향은 있을 수 있어요. 기초연금을 장기간 받아 금융자산이 쌓인 경우, 가구원 전체 재산 합계가 2억4,000만원에 근접할 수 있어요. 재산이 1억7,000만원 이상이면 근로장려금이 50% 감액되고, 2억4,000만원 이상이면 신청 자격 자체가 없어지니 재산 현황을 확인하는 게 좋아요.",
      },
      {
        title: "두 제도를 동시에 수급하는 방법은 무엇인가요?",
        content:
          "기초연금과 근로장려금을 동시에 수급하려면 각각의 신청 절차를 별도로 거쳐야 해요. 기초연금은 만 65세 생일이 속한 달의 한 달 전부터 주소지 읍·면·동 주민센터에서 신청하거나, 복지로(www.bokjiro.go.kr)나 국민연금공단에서 신청할 수 있어요. 한 번 신청하면 자격 유지 시 계속 지급돼요.\n\n근로장려금은 매년 5월 정기신청(홈택스·손택스·ARS 전화·세무서 방문)을 해야 해요. 65세 이상 어르신은 근로장려금 자동신청 제도(2025년부터 전 연령 확대)를 활용하면 한 번 동의 후 2년간 자동 신청이 돼요. 자동신청 동의는 홈택스, 손택스, ARS(1544-9944), 세무서에서 할 수 있어요.\n\n기초연금 소득인정액 계산 시, 근로장려금 수령액은 금융재산으로 반영될 수 있어요. 다만 기초연금 선정 기준액이 단독가구 월 228만원으로 상당히 넓은 범위이고, 근로장려금의 연간 지급액을 월 환산해도 크지 않아 기초연금 자격에 영향을 미치는 경우는 드물어요. 불안하다면 국민연금공단 콜센터(1355)에 문의해 개별 상황을 확인하세요.",
      },
      {
        title: "65세 이상 어르신에게 두 제도 중 어느 것이 더 중요한가요?",
        content:
          "65세 이상 어르신에게는 두 제도 모두 중요하지만, 성격이 달라서 상황에 따라 달라요. 은퇴 후 소득이 없거나 매우 적은 어르신에게는 기초연금이 핵심적인 생활비 보조 역할을 해요. 매월 정기적으로 지급되어 생활의 안정성을 높여줘요.\n\n반면, 고령에도 아르바이트·소규모 사업·농업 등으로 소득 활동을 하는 어르신에게는 근로장려금이 추가적인 소득 보조 수단이 돼요. 근로소득이 연 400만원~900만원 구간에 있으면 단독가구 최대 165만원을 수령할 수 있어요. 이 금액은 기초연금 4~5개월치에 해당하는 수준이에요.\n\n현재 정부는 근로를 통한 빈곤 탈출을 장려하기 위해 두 제도의 병행 수급을 허용하고 있어요. 65세 이상 어르신은 기초연금 자동 신청과 근로장려금 자동신청 제도를 모두 활용하면 매년 별도 신청의 번거로움 없이 두 혜택을 지속적으로 받을 수 있어요. 국민연금공단과 국세청 양쪽에 자동신청 동의를 해두는 것을 권장해요.",
        infoItems: [
          {
            type: "활용 전략",
            name: "기초연금",
            role: "읍면동 주민센터 또는 복지로에서 만 65세 생일 전월 신청",
          },
          {
            type: "활용 전략",
            name: "근로장려금",
            role: "매년 5월 신청 또는 자동신청 동의(홈택스·ARS) 활용",
          },
          {
            type: "자동신청",
            name: "근로장려금 자동신청",
            amount: "2년 유효",
            role: "홈택스·손택스·ARS·세무서에서 동의, 65세 이상 적극 활용 권장",
          },
        ],
      },
    ],
    datePublished: "2026-02-26",
    dateModified: "2026-02-26",
  },

  "vs-unemployment": {
    slug: "vs-unemployment",
    categorySlug: "comparisons",
    title: "근로장려금 vs 실업급여 중복 수령 | 동시 수급 조건과 소득 산정 구조",
    h1: "근로장려금 vs 실업급여 중복 수령 | 동시 수급 조건과 소득 산정 구조",
    metaDescription:
      "실업급여 수급 중 근로장려금을 신청할 수 있는지, 실업급여가 근로장려금 소득에 포함되는지 안내해요. 국민취업지원제도 구직촉진수당과의 중복 여부도 확인하세요.",
    description:
      "실업급여를 받는 중에도 근로장려금을 신청할 수 있을까요? 두 제도의 소득 산정 방식과 중복 수급 가능 조건을 안내해요.",
    heroDescription:
      "실업급여 받으면서 근로장려금도 받을 수 있을까요? 소득 포함 여부와 동시 수급 조건을 쉽게 설명해 드려요!",
    faq: [
      {
        question: "실업급여 수령 중에 근로장려금을 신청할 수 있나요?",
        answer:
          "근로장려금은 전년도 소득을 기준으로 신청해요. 따라서 실업급여를 받는 올해에 신청하는 근로장려금은 지난해 근로·사업소득을 기준으로 판단돼요. 지난해에 근로·사업소득이 있었고 요건을 충족한다면, 올해 실업급여를 받는 중에도 근로장려금을 신청할 수 있어요.",
      },
      {
        question: "실업급여가 근로장려금 총소득에 포함되나요?",
        answer:
          "아니요, 실업급여는 비과세소득이라 근로장려금의 총소득 계산에 포함되지 않아요. 근로장려금 총소득은 근로소득·사업소득·종교인소득·이자소득·배당소득·연금소득만 해당돼요. 실업급여(구직급여)와 취업촉진수당은 이 항목에 해당하지 않아요.",
      },
      {
        question: "국민취업지원제도 구직촉진수당도 근로장려금과 중복 가능한가요?",
        answer:
          "네, 국민취업지원제도의 구직촉진수당은 고용노동부가 지급하는 취업 지원 급여로, 근로장려금과는 별개의 제도예요. 구직촉진수당 역시 비과세소득에 해당하여 근로장려금 총소득에 포함되지 않으므로 중복 수급이 가능해요.",
      },
    ],
    sections: [
      {
        title: "실업급여와 근로장려금은 어떤 제도인가요?",
        content:
          "실업급여는 '고용보험법'에 따라 고용보험에 가입된 근로자가 비자발적으로 이직(퇴직)한 경우, 재취업 활동 기간 중 고용노동부가 지급하는 급여예요. 구직급여(기본 실업급여)는 퇴직 전 평균임금의 60%를 일당으로 계산해 최소 60일에서 최대 270일 동안 지급해요. 2026년 기준 1일 상한액은 6만6,000원이며, 하한액은 최저임금의 80%예요.\n\n근로장려금은 전년도에 근로·사업·종교인소득이 있는 저소득 가구에 국세청이 세금 환급 방식으로 지급하는 제도예요. 당해 연도 소득이 아닌 전년도 소득을 기준으로 신청·지급되는 점이 중요해요. 2026년 5월에 신청하는 근로장려금은 2025년도 소득을 기준으로 판단해요.\n\n두 제도는 운영 기관(고용노동부 vs 국세청), 소득 기준(고용보험 가입 여부 vs 전년도 총소득), 지급 방식(일별 지급 vs 연간·반기 지급)이 모두 달라요. 성격도 달라서 실업급여는 소득 중단 시 생활 안정을 위한 급여이고, 근로장려금은 근로 활동에 대한 소득 보조이에요. 이 차이로 인해 두 제도는 법적으로 중복 수급을 제한하지 않아요.",
        infoItems: [
          {
            type: "제도 비교",
            name: "실업급여",
            role: "고용노동부 운영, 비자발적 이직자 대상, 고용보험 가입 필수",
          },
          {
            type: "제도 비교",
            name: "근로장려금",
            role: "국세청 운영, 전년도 근로·사업소득 기준, 세금 환급형",
          },
          {
            type: "소득 기준",
            name: "실업급여",
            role: "퇴직 전 평균임금 60% × 지급일수 (최소 60일~최대 270일)",
          },
          {
            type: "소득 기준",
            name: "근로장려금",
            role: "전년도 총소득 기준 (단독 2,200만원 / 홑벌이 3,200만원 / 맞벌이 4,400만원 미만)",
          },
        ],
      },
      {
        title: "실업급여 수급 중 근로장려금은 어떻게 신청하나요?",
        content:
          "실업급여를 받는 중에 근로장려금을 신청하는 방법은 일반적인 신청 방법과 동일해요. 매년 5월에 홈택스·손택스·ARS(1544-9944)·세무서 방문 중 편한 방법으로 신청하면 돼요. 신청 시 기준은 전년도(2025년) 소득이므로, 2026년 현재 실업급여를 받고 있는 사실은 신청 자격에 직접적인 영향을 주지 않아요.\n\n근로장려금 신청 시 주의할 점은 총소득 요건이에요. 전년도에 근로소득·사업소득이 있었고 요건을 충족하면 신청 가능해요. 다만, 전년도 소득이 너무 낮아 요건을 충족하지 못하거나 반대로 너무 높아 한도를 초과한 경우에는 신청 자격이 없어요. 모의계산기를 통해 전년도 소득으로 예상 금액을 미리 확인해 보세요.\n\n실업 상태인 경우 당해 연도(2026년)에 근로·사업소득이 없으면, 다음 해(2027년) 5월에 신청하는 근로장려금(2026년 소득 기준)은 받지 못할 가능성이 높아요. 재취업 이후 소득이 발생해야 그 소득을 기반으로 근로장려금 신청 자격이 생겨요.",
      },
      {
        title: "국민취업지원제도와 근로장려금도 중복 가능한가요?",
        content:
          "국민취업지원제도의 구직촉진수당과 근로장려금은 중복 수급이 가능해요. 국민취업지원제도는 고용노동부가 운영하는 취업 지원 프로그램으로, 1유형 참여자는 월 50만원씩 최대 6개월(총 300만원)의 구직촉진수당을 받아요. 이 수당은 비과세소득으로 분류되어 근로장려금의 총소득 계산에 포함되지 않아요.\n\n국민취업지원제도에 참여하면서 이전 연도에 근로·사업소득이 있었다면, 5월 근로장려금 신청 자격이 생길 수 있어요. 예를 들어 2025년에 아르바이트로 연 500만원의 근로소득을 얻고 2026년 초 국민취업지원제도에 참여 중이라면, 2026년 5월에 2025년 소득 기준으로 근로장려금을 신청할 수 있어요.\n\n또한 국민취업지원제도 참여 중 취업에 성공하여 취업성공수당(최대 150만원)을 받는 경우도 있어요. 취업성공수당 역시 비과세소득으로 근로장려금 총소득에 포함되지 않아요. 취업 후 근로소득이 발생하면 그다음 해 근로장려금 신청 대상이 될 수 있어요.",
        infoItems: [
          {
            type: "중복 가능 여부",
            name: "실업급여(구직급여)",
            amount: "중복 가능",
            role: "비과세소득, 근로장려금 총소득 미포함",
          },
          {
            type: "중복 가능 여부",
            name: "구직촉진수당",
            amount: "중복 가능",
            role: "비과세소득, 월 50만원 × 최대 6개월",
          },
          {
            type: "중복 가능 여부",
            name: "취업성공수당",
            amount: "중복 가능",
            role: "비과세소득, 취업 시 최대 150만원",
          },
        ],
      },
      {
        title: "재취업 후 근로장려금 신청은 언제부터 가능한가요?",
        content:
          "재취업 후 근로장려금 신청은 재취업한 연도의 다음 해 5월부터 가능해요. 근로장려금은 전년도 소득을 기준으로 하므로, 재취업하여 소득이 발생한 해의 다음 해에 신청할 수 있어요. 예를 들어 2026년 3월에 재취업했다면, 2026년 소득을 기준으로 2027년 5월에 근로장려금을 신청할 수 있어요.\n\n재취업 후 소득이 연간 기준으로 매우 낮을 수 있어요. 예를 들어 2026년 3월에 재취업하여 12월까지 9개월 치 월급을 받으면 연간 총소득이 평소보다 적어요. 이 경우 근로장려금 산정 시 실제 수령액이 높아질 수 있어요. 연간 근로소득이 낮을수록 점증 구간에 해당하여 소득 대비 장려금 비율이 높아지는 구조예요.\n\n반기신청을 활용하면 재취업 후 더 빨리 장려금을 수령할 수도 있어요. 재취업 연도의 상반기(1월~6월) 근로소득이 있다면, 같은 해 9월에 반기신청을 통해 예상 장려금의 35%를 먼저 수령할 수 있어요. 단, 반기신청은 근로소득자만 가능하며 사업소득자는 정기신청만 할 수 있어요.",
      },
    ],
    datePublished: "2026-02-26",
    dateModified: "2026-02-26",
  },

  "vs-child-allowance": {
    slug: "vs-child-allowance",
    categorySlug: "comparisons",
    title: "근로장려금 vs 아동수당 비교 | 자녀장려금 포함 세 제도 중복 수급 안내",
    h1: "근로장려금 vs 아동수당 비교 | 자녀장려금 포함 세 제도 중복 수급 안내",
    metaDescription:
      "아동수당·자녀장려금·근로장려금 세 제도의 지급 대상, 금액, 중복 수급 가능 여부를 비교해요. 자녀가 있는 가구의 최대 수혜 조합을 확인하세요.",
    description:
      "아동수당·자녀장려금·근로장려금 세 제도의 차이와 중복 수급 가능 여부를 안내해요. 자녀 있는 가구의 최대 혜택 조합을 알아보세요.",
    heroDescription:
      "아동수당, 자녀장려금, 근로장려금 모두 받을 수 있을까요? 세 제도의 차이와 중복 수급 구조를 한눈에 비교해 드려요!",
    faq: [
      {
        question: "아동수당, 자녀장려금, 근로장려금을 모두 받을 수 있나요?",
        answer:
          "세 제도 모두 별개의 법률에 따라 운영되므로 동시 수급이 가능해요. 아동수당은 행정안전부(읍면동)에 신청, 자녀장려금과 근로장려금은 국세청(홈택스·손택스)에 신청하면 돼요. 각 제도의 요건을 충족하면 세 가지를 모두 받을 수 있어요.",
      },
      {
        question: "아동수당은 소득에 관계없이 받을 수 있나요?",
        answer:
          "네, 아동수당은 2019년부터 소득·재산과 관계없이 만 8세 미만 모든 아동에게 보편 지급돼요. 부모의 소득이 높아도, 기초생활수급자여도 만 8세 미만 아동은 월 10만원을 받을 수 있어요.",
      },
      {
        question: "자녀장려금과 근로장려금은 같이 신청하나요?",
        answer:
          "네, 자녀장려금과 근로장려금은 5월 정기신청 기간에 홈택스·손택스에서 한 번에 신청할 수 있어요. 신청서에 두 제도의 항목이 함께 있어 별도로 신청할 필요가 없어요. 각 제도의 요건을 충족하면 모두 지급돼요.",
      },
    ],
    sections: [
      {
        title: "세 제도의 기본 구조는 어떻게 다른가요?",
        content:
          "아동수당은 '아동수당법'에 따라 보건복지부가 운영하는 보편 아동 지원 제도예요. 만 8세 미만(0세~95개월) 아동이 있는 가구에 아동 1인당 월 10만원을 지급해요. 소득·재산 요건이 없는 보편급여로, 대한민국 국적 또는 외국인등록(만 60일 이상 체류)을 한 아동이면 모두 받을 수 있어요. 출생신고 후 읍·면·동 주민센터 또는 복지로·정부24에서 신청하면 돼요.\n\n자녀장려금은 '조세특례제한법'에 따라 국세청이 운영하는 자녀 양육 지원 세제 혜택이에요. 만 18세 미만 부양자녀가 있는 가구 중 소득·재산 요건을 충족하는 경우 자녀 1인당 연 최대 100만원을 지급해요. 2026년 기준 총소득 요건은 홑벌이가구 4,000만원 미만, 맞벌이가구 4,000만원 미만이에요. 근로장려금과 동일한 재산 요건(2억4,000만원 미만)이 적용돼요.\n\n근로장려금은 가구 단위로 지급되며 자녀 수에 무관하게 가구 유형(단독·홑벌이·맞벌이)과 총소득에 따라 금액이 결정돼요. 자녀장려금은 자녀 수에 따라 금액이 달라져요. 아동수당은 아동 개인 단위로 지급돼요. 세 제도 모두 중복 수급이 가능해요.",
        infoItems: [
          {
            type: "제도 비교",
            name: "아동수당",
            amount: "월 10만원/아동",
            role: "만 8세 미만, 소득 무관 보편 지급, 보건복지부",
          },
          {
            type: "제도 비교",
            name: "자녀장려금",
            amount: "연 최대 100만원/자녀",
            role: "만 18세 미만, 소득·재산 요건 있음, 국세청",
          },
          {
            type: "제도 비교",
            name: "근로장려금",
            amount: "연 최대 330만원/가구",
            role: "가구 단위, 자녀 수 무관, 소득·재산 요건, 국세청",
          },
        ],
      },
      {
        title: "자녀장려금의 소득·재산 요건은 어떻게 되나요?",
        content:
          "자녀장려금은 2026년 기준으로 홑벌이가구·맞벌이가구 모두 총소득 4,000만원 미만이 요건이에요. 단독가구는 자녀장려금 대상이 아니에요. 자녀장려금의 재산 요건은 근로장려금과 동일하게 가구원 전체 재산 합계 2억4,000만원 미만이며, 1억7,000만원 이상이면 50% 감액돼요.\n\n자녀장려금의 자녀 1인당 지급액은 가구 총소득 구간에 따라 달라요. 홑벌이가구를 기준으로 총소득 2,100만원 미만(점증 구간)에서는 총소득 구간별로 산정되고, 2,100만원 이상 ~ 4,000만원 미만(점감 구간)에서는 100만원에서 감액 비율을 적용해요. 자녀 2명이면 최대 200만원, 3명이면 최대 300만원까지 받을 수 있어요.\n\n자녀장려금의 '부양자녀'는 만 18세 미만으로 연간 소득금액이 100만원 이하인 자녀를 말해요. 거주 여부보다 생계를 같이 하는 가족 관계가 중요해요. 입양아, 위탁아동도 요건에 따라 포함될 수 있어요. 근로장려금과 달리 자녀 수에 따라 지급액이 달라지므로 자녀가 많을수록 자녀장려금의 혜택이 커요.",
        infoItems: [
          {
            type: "소득 요건",
            name: "자녀장려금",
            amount: "4,000만원 미만",
            role: "홑벌이·맞벌이 모두 동일 (단독가구 해당 없음)",
          },
          {
            type: "재산 요건",
            name: "자녀장려금",
            amount: "2.4억원 미만",
            role: "근로장려금과 동일 (1.7억원 이상 50% 감액)",
          },
          {
            type: "지급 금액",
            name: "자녀 1명",
            amount: "최대 100만원",
            role: "소득 구간에 따라 산정",
          },
          {
            type: "지급 금액",
            name: "자녀 2명",
            amount: "최대 200만원",
            role: "자녀 수 × 100만원 (소득 구간 적용)",
          },
        ],
      },
      {
        title: "세 제도를 모두 받으면 얼마나 수령할 수 있나요?",
        content:
          "세 제도를 모두 활용하면 자녀 수와 소득 수준에 따라 상당한 금액을 받을 수 있어요. 예를 들어, 홑벌이가구에서 만 5세, 만 10세 두 자녀가 있고, 총소득이 2,000만원인 경우를 살펴볼게요. 아동수당은 만 8세 미만인 5세 자녀에게 월 10만원 × 12개월 = 연 120만원을 받아요.\n\n자녀장려금은 두 자녀(만 18세 미만)에 대해 소득 구간(2,000만원)에서 산정하면 자녀 1인당 약 85만원~100만원 수준이 돼요. 두 자녀 합산 약 170만원~200만원을 받을 수 있어요. 근로장려금은 홑벌이가구 총소득 2,000만원 구간에서 최대 285만원 중 점감 구간 계산으로 약 220만원~280만원을 받을 수 있어요.\n\n세 제도를 합산하면 연간 약 510만원~600만원에 달하는 혜택을 받을 수 있어요. 물론 정확한 금액은 실제 소득과 재산 상황에 따라 달라지므로, 홈택스 모의계산기나 이 사이트의 계산기를 활용해 실제 예상액을 계산해 보세요. 세 제도 중 아동수당은 주민센터에서, 근로장려금과 자녀장려금은 국세청(홈택스·손택스)에서 한 번에 신청할 수 있어요.",
      },
      {
        title: "아동수당이 근로장려금 수급에 영향을 주나요?",
        content:
          "아동수당은 근로장려금 총소득 계산에 포함되지 않아요. 아동수당은 소득세법상 비과세소득으로, 근로장려금의 총소득 산정 항목(근로·사업·종교인·이자·배당·연금소득)에 해당하지 않아요. 매월 10만원의 아동수당을 받아도 근로장려금 소득 요건에는 영향을 주지 않아요.\n\n반대로 근로장려금 수령이 아동수당에 영향을 주지도 않아요. 아동수당은 소득·재산과 무관한 보편급여이므로, 근로장려금을 수령했다고 해서 아동수당이 줄거나 중단되는 일은 없어요. 두 제도는 완전히 독립적으로 작동해요.\n\n자녀장려금과 근로장려금은 같은 국세청에서 운영하는 제도이지만 서로 영향을 주지 않아요. 각각 독립된 요건과 산정 방식을 가지고 있어요. 다만, 근로장려금을 신청할 때 자녀 정보를 함께 제출하면 자녀장려금 자격도 같이 심사되어 한 번의 신청으로 두 가지 모두 처리할 수 있어 편리해요.",
        infoItems: [
          {
            type: "영향 여부",
            name: "아동수당 → 근로장려금",
            amount: "영향 없음",
            role: "아동수당은 비과세소득, 총소득 미포함",
          },
          {
            type: "영향 여부",
            name: "근로장려금 → 아동수당",
            amount: "영향 없음",
            role: "아동수당은 소득 무관 보편급여",
          },
          {
            type: "영향 여부",
            name: "자녀장려금 ↔ 근로장려금",
            amount: "독립 산정",
            role: "동시 신청 가능, 각각 별도 요건 심사",
          },
        ],
      },
    ],
    datePublished: "2026-02-26",
    dateModified: "2026-02-26",
  },

  "regular-vs-semi-detail": {
    slug: "regular-vs-semi-detail",
    categorySlug: "comparisons",
    title: "근로장려금 정기 vs 반기 어떤 게 유리? | 신청 방식별 지급 시기·금액 비교",
    h1: "근로장려금 정기 vs 반기 어떤 게 유리? | 신청 방식별 지급 시기·금액 비교",
    metaDescription:
      "근로장려금 정기신청과 반기신청의 지급 시기, 금액, 대상 요건을 비교해요. 직장인·사업소득자·현금 필요 시점에 따라 유리한 신청 방법을 선택하세요.",
    description:
      "근로장려금 정기신청(5월)과 반기신청(9월·3월)의 차이를 비교해요. 어떤 방식이 나에게 더 유리한지 한눈에 확인하세요.",
    heroDescription:
      "근로장려금 정기신청과 반기신청 중 어느 게 더 유리할까요? 지급 시기·금액·가능 대상을 비교해 최적의 선택을 도와드려요!",
    faq: [
      {
        question: "정기신청과 반기신청의 가장 큰 차이는 무엇인가요?",
        answer:
          "정기신청은 5월에 한 번 신청해 9월에 전액(100%)을 한 번에 받는 방식이에요. 반기신청은 상반기분(9월 신청 → 12월 지급)과 하반기분(3월 신청 → 6월 지급)으로 나눠 각 35%씩 먼저 받고, 다음 해 정기신청에서 나머지를 정산받아요. 정기가 전액 한 번에, 반기는 조기에 나눠 받는 방식이에요.",
      },
      {
        question: "사업소득자도 반기신청을 할 수 있나요?",
        answer:
          "아니요, 반기신청은 근로소득자(고용보험 가입 또는 원천징수 근로소득자)만 가능해요. 사업소득자, 농업·임업·어업 소득자, 종교인소득자는 정기신청만 가능해요. 근로소득과 사업소득이 함께 있는 혼합 소득자도 사업소득이 있으면 반기신청 불가 여부를 국세청에 확인해야 해요.",
      },
      {
        question: "반기신청으로 받은 금액이 실제보다 많으면 어떻게 되나요?",
        answer:
          "반기신청으로 선지급된 금액(35% × 2회)이 정기신청 정산 시 확정된 최종 금액보다 많으면 초과분을 반환해야 해요. 이를 '과다지급'이라 하며, 다음 해 정기신청 정산 과정에서 자동으로 차감되거나 별도 고지로 반환 요청이 올 수 있어요.",
      },
    ],
    sections: [
      {
        title: "정기신청과 반기신청의 구조는 어떻게 다른가요?",
        content:
          "정기신청은 매년 5월 1일부터 6월 1일까지 전년도 소득을 기준으로 근로장려금을 신청하는 방식이에요. 신청 후 국세청 심사를 거쳐 9월 말까지 장려금 전액(100%)이 한 번에 지급돼요. 정기신청은 근로소득자, 사업소득자, 종교인소득자 등 모든 소득 유형의 신청자가 이용할 수 있는 가장 기본적인 신청 방법이에요.\n\n반기신청은 당해 연도 상반기·하반기 소득을 기준으로 중간에 장려금을 미리 받는 방식이에요. 상반기분은 9월 1일~15일에 신청해 12월에 지급되고, 하반기분은 다음 해 3월 1일~15일에 신청해 6월에 지급돼요. 각 반기에 예상 장려금의 35%씩(총 70%)을 선지급받고, 나머지 30%는 다음 해 5월 정기신청을 통해 정산받아요.\n\n반기신청으로 받은 총 선지급액(35% × 2회 = 70%)이 최종 확정 장려금보다 많으면 초과분을 반환해야 하고, 적으면 나머지를 추가로 받아요. 반기신청 후에도 다음 해 5월에 정기신청을 하면 최종 정산이 이루어져요. 반기신청을 하지 않으면 정기신청으로만 처리돼요.",
        infoItems: [
          {
            type: "신청 방식",
            name: "정기신청",
            role: "5월 1일~6월 1일 신청 → 9월 지급(100%), 모든 소득 유형 가능",
          },
          {
            type: "신청 방식",
            name: "반기신청(상반기)",
            role: "9월 1일~15일 신청 → 12월 지급(35%), 근로소득자만 가능",
          },
          {
            type: "신청 방식",
            name: "반기신청(하반기)",
            role: "3월 1일~15일 신청 → 6월 지급(35%), 근로소득자만 가능",
          },
          {
            type: "정산",
            name: "반기신청 후 정산",
            role: "다음 해 5월 정기신청에서 최종 정산 (초과분 반환 or 추가 수령)",
          },
        ],
      },
      {
        title: "정기신청이 유리한 경우는 언제인가요?",
        content:
          "정기신청이 유리한 경우는 소득이 안정적인 직장인, 사업소득이 있는 경우, 연간 소득 변동이 큰 경우 등이에요. 가장 단순하고 확실한 방법으로, 5월에 한 번 신청하면 9월에 전액을 한 번에 받을 수 있어요. 복잡한 중간 정산 과정이 없어서 관리가 편해요.\n\n사업소득자, 종교인소득자, 농업·임업·어업 소득자는 반기신청 자체가 불가능하므로 정기신청을 이용해야 해요. 이 소득 유형의 경우 연도 중 소득이 불규칙하게 발생하는 경우가 많아, 중간에 선지급받으면 나중에 초과분을 반환해야 하는 상황이 생길 수 있어요.\n\n연간 소득 변동이 큰 경우에도 정기신청이 안전해요. 예를 들어 연초에 소득이 많다가 하반기에 줄어드는 경우, 반기신청으로 예상 금액의 35%를 받았다가 최종 확정액이 선지급액보다 적으면 반환 부담이 생겨요. 정기신청은 확정된 전년도 소득을 기준으로 하므로 이런 불확실성이 없어요.",
        infoItems: [
          {
            type: "정기신청 추천",
            name: "사업소득자",
            role: "반기신청 불가 대상 (정기신청만 가능)",
          },
          {
            type: "정기신청 추천",
            name: "소득 변동 큰 경우",
            role: "반기 정산 시 초과분 반환 위험 회피",
          },
          {
            type: "정기신청 추천",
            name: "관리 편의성",
            role: "연 1회 신청, 9월 전액 일시 수령, 정산 불필요",
          },
        ],
      },
      {
        title: "반기신청이 유리한 경우는 언제인가요?",
        content:
          "반기신청이 유리한 경우는 빠른 현금이 필요할 때, 근로소득만 있는 안정적인 직장인인 경우예요. 9월(상반기분)과 6월(하반기분)에 나눠 받으면, 정기신청의 9월보다 더 빨리 일부 금액을 수령할 수 있어요. 생활비가 급하거나 연 중 목돈이 필요한 시점이 있다면 반기신청이 유용해요.\n\n근로소득만 있고 소득이 비교적 안정적인 직장인에게 반기신청이 가장 적합해요. 매월 일정한 급여를 받는 직장인은 상·하반기 소득이 균등하게 발생하므로, 반기별로 35%씩 선지급받고 정산할 때 초과분이 발생할 가능성이 낮아요. 최종적으로 받는 총액은 정기신청과 거의 동일해요.\n\n반기신청을 활용하면 연 3회에 걸쳐 장려금을 수령하는 효과가 있어요. 상반기 9월 신청 후 12월 지급(35%) → 하반기 3월 신청 후 6월 지급(35%) → 정기신청 정산 후 9월 지급(나머지 30%)의 순서로 받게 돼요. 이 방식은 현금 흐름 관리에 유리하고, 정산 시 추가 금액도 받을 수 있어요.",
        infoItems: [
          {
            type: "반기신청 추천",
            name: "안정적 직장인",
            role: "근로소득만 있고 소득 변동 적은 경우 적합",
          },
          {
            type: "반기신청 추천",
            name: "빠른 현금 필요",
            role: "12월·6월 분할 수령으로 조기 현금 확보",
          },
          {
            type: "반기신청 추천",
            name: "현금흐름 관리",
            role: "연 3회(12월·6월·9월) 분산 수령 효과",
          },
        ],
      },
      {
        title: "반기신청 시 정산 과정과 주의사항은 무엇인가요?",
        content:
          "반기신청을 했더라도 다음 해 5월에 정기신청을 하면 최종 장려금이 확정돼요. 정기신청에서 확정된 금액과 반기신청으로 받은 금액(35% × 2회 = 70%)의 차이를 정산해요. 최종 확정액이 더 많으면 차액을 추가로 받고, 더 적으면 초과분을 반환해야 해요.\n\n정산 시 추가 수령하는 금액은 약 최종 확정액의 30%(미지급분)에 해당해요. 하지만 소득이 예상보다 많아 최종 확정액이 줄어든 경우, 이미 선지급받은 금액이 최종 확정액을 초과할 수 있어요. 이 경우 다음 해 9월 지급분에서 차감하거나 별도 고지로 반환 요청을 받아요.\n\n반기신청 대상자 요건도 확인해야 해요. 반기신청은 상반기(1월~6월) 또는 하반기(7월~12월)에 근로소득이 발생한 경우에만 신청할 수 있어요. 해당 반기에 근로소득이 없었다면 해당 반기 신청이 불가해요. 또한, 반기신청 기간(9월 1일~15일, 3월 1일~15일)이 정기신청 기간(5월)보다 짧아서 기간을 놓치지 않도록 주의해야 해요.",
        visual: {
          type: "steps",
          items: [],
          steps: [
            {
              label: "상반기 신청 (9월)",
              description: "9월 1~15일 신청 → 12월 지급 (예상액 35%)",
            },
            {
              label: "하반기 신청 (3월)",
              description: "3월 1~15일 신청 → 6월 지급 (예상액 35%)",
            },
            {
              label: "정기신청 정산 (5월)",
              description: "5월 신청 → 9월 최종 정산 (나머지 30% ± 차이)",
            },
          ],
        },
      },
      {
        title: "정기신청과 반기신청 중 어느 것을 선택해야 하나요?",
        content:
          "정기신청과 반기신청 선택 기준은 소득 유형, 소득의 안정성, 현금 필요 시점이에요. 사업소득자·종교인소득자·농업소득자는 선택권 없이 정기신청만 가능해요. 근로소득자는 두 가지 모두 선택할 수 있으므로 본인 상황에 맞게 선택하면 돼요.\n\n소득이 안정적이고 현금이 급하지 않다면 정기신청이 편해요. 한 번만 신청하면 되고, 9월에 전액을 한 번에 받아요. 정산 과정도 없어서 초과분 반환 걱정이 없어요. 근로장려금을 저축이나 연간 계획에 맞게 사용하려는 분에게 적합해요.\n\n현금이 빨리 필요하거나 연간 지출을 분산하고 싶은 분은 반기신청이 유리해요. 다만, 반기신청 후 소득이 예상보다 늘어난 경우 초과분을 반환해야 할 수 있으니, 연간 소득을 대략 예상할 수 있는 분에게 적합해요. 두 방식 모두 최종 수령액의 합계는 동일해요.",
        infoItems: [
          {
            type: "선택 기준",
            name: "정기신청 선택",
            role: "사업소득자, 소득 불안정, 관리 편의 우선, 9월 전액 수령 선호",
          },
          {
            type: "선택 기준",
            name: "반기신청 선택",
            role: "근로소득자만, 안정적 소득, 빠른 현금 필요, 분산 수령 선호",
          },
          {
            type: "주의사항",
            name: "반기신청 기간",
            role: "상반기: 9월 1~15일 / 하반기: 3월 1~15일 (기간 짧음)",
          },
          {
            type: "주의사항",
            name: "초과 지급",
            role: "반기 선지급액 > 최종 확정액이면 초과분 반환 필요",
          },
        ],
      },
    ],
    datePublished: "2026-02-26",
    dateModified: "2026-02-26",
  },
};
