 ...
 *
 * robots.txt만 보존하고 나머지 .txt는 모두 삭제
 */
...
function cleanDir(dir) {
  for (const entry of readdirSync(dir)) {
    ...
    if (stat.isDirectory()) {
      cleanDir(fullPath);
    } else if (entry.endsWith(".txt") && !entry.startsWith("robots") && !entry.startsWith("a25a1b18")) {
      // robots.txt 와 IndexNow 키 파일(a25a1b18...txt)은 보존
      unlinkSync(fullPath);
      deleted++;
    }
  }
